...
class WEIGHTANDBALANCE(Base):
    ...
    def __init__(self, *args, **kwargs):
        ...
        def_opts = {"dtype": [str, "d"], "t": [float, 0.001], "rho": [float, 2600], "inertiaModifier": [float, 1.5]}
        informs = {}
            ...
            if isinstance(acg[i], LiftingSurface):
                ...
                if acg[i].Name.lower() == "wing":
                    ...
                    for j in range(nseg):
                        ...
                        # new xC4 computation
                        temp1 = (1 + 2 * Taper) * ((ytLE - yrLE) / sumSpan) * numpy.tan(SweepLE)
    ...
    def wingCGDriver(self, acg, geom):
        ...
        xcg = self.calculateWingCenterOfGravity(geom.ForeSparPercent, geom.RearSparPercent, geom.CGPercent, MAC, C4MAC)
            ...
            if isinstance(acg[i], LiftingSurface):
                ...
                if acg[i].Name.lower() == "wing":
                    ...
                    for j in range(nseg):
                        ...
                        # Volume???
                        V = Span * (
                            t_root * (C_root + (Span / 2.0) * (numpy.tan(SweepTE) - numpy.tan(SweepLE)))
                            - (
                                (t_root - t_tip)
                                * ((C_root / 2.0) + (Span / 3.0) * (numpy.tan(SweepTE) - numpy.tan(SweepLE)))
                            )
                        )
                        # print 'V',V
                        ...
                        I1x = (W * Span**3 / (3 * V)) * (
                            ...
                            + (
                                t_root
                                * (C_root / 3.0 + (Span * numpy.tan(SweepTE) / 4.0) - Span * numpy.tan(SweepLE) / 4.0)
                            )
                        )
                        ...
                        acg[i][j].y_Centroid = (Span**2 / V) * (
                            (
                                ...
                                - (t_root - t_tip)
                                * ((C_root / 3.0) + (Span / 4.0) * (numpy.tan(SweepTE) - numpy.tan(SweepLE)))
                            )
                        )
    ...
    def calculateWingCenterOfGravityDerivatives(self, x, geo, acg, geom):
        ...
        for key in xw.keys():
            ...
            for i in range(len(xw[key])):
                ...
                xcg = self.calculateWingCenterOfGravity(
                    geom.ForeSparPercent, geom.RearSparPercent, geom.CGPercent, MAC, C4MAC
                )
