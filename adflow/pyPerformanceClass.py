...
class PERFORMANCE(object):
    ...
    def calculateFrequencyAndDamping(self, Cmq, Clalpha, Cd, Cmalpha, Cmalphadot, mass, Iy, rho, Area, U, c):
        """
    ...
    def thumbprintDriver(self, acg, wbc, geom, averagesol, rho, V, A, thickness):
        ...
        [Wn, DampingRatio] = self.calculateFrequencyAndDamping(
            averagesol["cmq"],
            averagesol["clalpha"],
            averagesol["cd0"],
            averagesol["cmalpha"],
            averagesol["cmalphadot"],
            m,
            Iy,
            rho,
            A,
    ...
    def CAPDerivativeDriver(self, x, geo, con, acg, wbc, geom, dvFunc, rho, V, A):
        ...
        for key in xw.keys():
            ...
            for i in range(len(xw[key])):
                ...
                # end
                CAP, Damp = self.CAPDriver(
                    acg, wbc, geom, averagesol, rho, V, A, thick_con_c.reshape(con.thickConSizes[0])
                )
    ...
    def CAPDerivativeDriverpyGeo(self, x, geo, con, acg, wbc, geom, dvFunc, rho, V, A, surface, surfInst):
        ...
        for key in xw.keys():
            ...
            for i in range(len(xw[key])):
                ...
                # print 'thick',type(thick_con_c),thick_con_c,dCondx[:,geo.DV_namesGlobal[key]]*deltax
                # end
                CAP, Damp = self.CAPDriverpyGeo(acg, wbc, geom, averagesol, rho, V, A, surfInst)
    ...
    def CAPDerivativeDriverpyGeoFD(self, x, geo, con, acg, wbc, geom, dvFunc, rho, V, A, surface, surfInst):
        ...
        surface.setCellCentroidCoordinates(surfInst, geo.update("Cent_coord"))
        CAPref, Dampref = self.CAPDriverpyGeo(acg, wbc, geom, averagesol, rho, V, A, surfInst)
        CAPderiv = {}
        Dampderiv = {}
        # geo.complex = True
        for key in xw.keys():
