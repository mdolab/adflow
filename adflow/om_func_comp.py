...
class OM_FUNC_COMP(ExplicitComponent):
    ...
    def compute_jacvec_product(self, inputs, d_inputs, d_outputs, mode):
        ...
        if mode == "fwd":
            ...
            funcsdot = solver.computeJacobianVectorProductFwd(xDvDot=xDvDot, wDot=wDot, funcDeriv=True)
            ...
            if "states" in d_inputs and n_input_vars == 1:
                wBar = solver.computeJacobianVectorProductBwd(funcsBar=funcsBar, wDeriv=True)
            elif ("states" not in d_input_vars) and n_input_vars:
                xDVBar = solver.computeJacobianVectorProductBwd(funcsBar=funcsBar, xDvDeriv=True)

            elif ("states" in d_input_vars) and n_input_vars:
                wBar, xDVBar = solver.computeJacobianVectorProductBwd(funcsBar=funcsBar, wDeriv=True, xDvDeriv=True)
