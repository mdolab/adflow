...
contains
  ...
  subroutine determineSubranges
    ...
    ! Step 2. Correct for possible rind layers. Rind layers are
    ! necessary for nodal output

    if( storeRindLayer .or. nodalOutput ) then
    ...
    solLoop: do mm=1,nSolVar
       ...
       do ll=1,nBlocks
          ...
          if( contributeToFace(ll) ) then
             ...
             call storeSurfsolInBuffer(&
                  ind, buffer, jj, ii, faceID, nodalRange(1,1,kk), &
                  cellRange(1,1,kk), solNames(mm), viscousSubface)
          endif
       enddo
