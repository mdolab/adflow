...
subroutine NKsolver_custom
  ...
  use NKSolverVars, only: dRdw,dRdwPre,ctx,jacobian_lag,&
       nksolversetup,rhoRes0,rhoresstart, &
       totalR0,totalRStart,itertot0,wVec,rVec,deltaW,reason,NKSolvedOnce,&
       ksp,ksp_rtol,ksp_atol,ksp_div_tol,ksp_max_it,ksp_subspace
...
#include "include/finclude/petsc.h"
  ...
  ! Its is (far) larger that necessary. The "iteration" limit is
  ! really set from the maxmimum number of funcEvals
  ...
  NonLinearLoop: do iter =1,maxNonLinearIts
     ...
     ! Set the new W, evalulate the residual and set the residual:
     call setW(wVec)
     ...
     ! Get the EW Forcing tolerance rtol
