subroutine setupNKsolver
  ...
  use ANKSolverVars, only: dRdw, dRdwPre,drdwpseudo, ctx, wVec, rVec, deltaW, &
        nksolversetup, newtonKrylovKSP, PETSC_TRUE, MAT_STRUCTURALLY_SYMMETRIC
  implicit none
#define PETSC_AVOID_MPIF_H
  ...
  if (.not. ANKSolverSetup) then
     ...
     call KSPCreate(SUMB_COMM_WORLD, ANKKSP, ierr)
     call EChk(ierr, __FILE__, __LINE__)

     ! Set operators for the solver

     call KSPSetOperators(ANKKSP, dRdwPre, dRdwPre, ierr)
     call EChk(ierr, __FILE__, __LINE__)
