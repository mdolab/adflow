...
subroutine SSTSolve(resOnly)
  ...
  real(kind=realType) :: LengthScaleSST, decayTermFunc, cDESkw_ov_cDESke, &
       cDESBlend  ! eran-des
  ...
  if(applyDES)then
     ...
     cDESkw_ov_cDESke = 0.78_realType /cDES
     do k=2,kl
        do j=2,jl
           do i=2,il
              ...
              ! equation. Note that prod currently contains the unscaled
              ! source term. Furthermore the production term of k is
              ! limited to a certain times the destruction term.

              rhoi = one/w(i,j,k,irho)
              ss   = prod(i,j,k)
              spk  = rev(i,j,k)*ss*rhoi
              !
              LengthScaleSST = sqrt(w(i,j,k,itu1) + eps )/(rSSTBetas*w(i,j,k,itu2) )
              !
              ! based on TURMB170, modify Cdes betwenn k-epsilon and k-omega value
              !
              cDESBlend = (one - f1(i,j,k)) + cDESkw_ov_cDESke * f1(i,j,k)
              decayTermFunc  = max(LengthScaleSST/(cDESBlend*filterDES(i,j,k)),one)
              !
              sdk  = rSSTBetas*w(i,j,k,itu1)*w(i,j,k,itu2)*decayTermFunc
              ...
              qq(i,j,k,1,1) = rSSTBetas*w(i,j,k,itu2)*decayTermFunc
              qq(i,j,k,1,2) = zero
              qq(i,j,k,2,1) = zero
