...
subroutine saSolve(resOnly)
  ...
  real(kind=realType) :: DDESrd, DDESfd, DDES_tilda_d  ! eran-des
  !
  ! --- eran-tran starts
  !

  real(kind=realType) :: modCb1, Xs, Xe, pp, transC
  !
  ! --- eran-tran ends
  !
  !
  ...
  if(forcedTransition)then
     ...
     Xs = xTransition - TransHLength
     Xe = xTransition + TransHLength
     pp = 1.5
     transC = one/(Xe-Xs)**pp
  ...
  if(applyDES)then
     ...
     !
     ! The original Spalart model (known also as DES97)   (applyDDES=false)
     !
     ! DDES (delayed DES, based on Spalart et al., Theor. Compt. Fluid Mech., 
     !       2006, vol 20, pp 181-195," A new version of detached eddy simulation, resistant
     !       to ambiguous grid definitions"               (applyDDES = true)
     !
     ! The whole triple-loop are rewrittne in order to avoid if statements inside the loop
     !
     !
     !--------- DES source term
     !
     if (.not.applyDDES)then
        !
        ! DES97 version
        !
        do k=2,kl
           do j=2,jl
              do i=2,il
                 ...
                 ! Take the square root of the production term to
                 ! obtain the correct production term.

                 ss = sqrt(prod(i,j,k))

                 ! Compute the laminar kinematic viscosity, the inverse
                 ! of wall distance squared, the ratio chi (ratio of
                 ! nuTilde and nu) and the functions fv1 and fv2. The
                 ! latter corrects the production term near a viscous
                 ! wall.

                 nu       = rlv(i,j,k)/w(i,j,k,irho)
                 dist2Inv = one/(filterDES(i,j,k)**2)
                 chi      = w(i,j,k,itu1)/nu
                 chi2     = chi*chi
                 chi3     = chi*chi2
                 fv1      = chi3/(chi3+cv13)
                 ...
                 ! solution laminar. When running in fully turbulent
                 ! mode this function should be set to 0.0.
                 ...
                 modCb1  = rsaCb1
                 if(forcedTransition) &
                      call forcedTransDecFunc(Xs,Xe,pp,transC,x(i,j,k,1),ft2,modCb1)
                 !
                 ! --eran-tran ends

                 ! Correct the production term to account for the
                 ! influence of the wall. Make sure that this term
                 ! remains positive (the function fv2 is negative
                 ! between chi = 1 and 18.4, which can cause sst to
                 ! go negative, which is undesirable).

                 sst = ss + w(i,j,k,itu1)*fv2*kar2Inv*dist2Inv
                 sst = max(sst,1.e-10_realType)

                 ! Compute the function fw. The argument rr is cut off
                 ! at 10 to avoid numerical problems. This is ok,
                 ! because the asymptotical value of fw is then already
                 ! reached.
                 ...
                 ! Compute the source term, if needed. Some terms are
                 ! save for the linearization.
                 !
                 !
                 ! --eran-tran starts modCb1 instead of rsaCb1
                 !
                 term1 = modCb1*(one-ft2)*ss
                 term2 = dist2Inv*(kar2Inv*modCb1*((one-ft2)*fv2 + ft2) &
                      -           rsaCw1*fwSa)
                 !
                 !--eran-tran ends
                 !
                 ...
                 ! Compute some derivatives w.r.t. nuTilde. These will
                 ! occur in the left hand side, i.e. the matrix for the
                 ! implicit treatment.
                 ...
                 ! containing term1 is treated explicitly. The reason is
                 ! that implicit treatment of this part leads to a
                 ! decrease of the diagonal dominance of the jacobian and
                 ! it thus decreases the stability. You may want to play
                 ! around and try to take this term into account in the
                 ! Jacobian. Note that -dsource/dnu is stored.

                 !--- eran-tran  : modCb1 instead of rsaCb1
                 qq(i,j,k) = -two*term2*w(i,j,k,itu1)                      &
                      -  dist2Inv*w(i,j,k,itu1)*w(i,j,k,itu1)         &
                      * (modCb1*kar2Inv*(dfv2-ft2*dfv2-fv2*dft2+dft2) &
                      -  rsaCw1*dfw)
        ...
        do k=2,kl
           do j=2,jl
              do i=2,il
                 ...
                 ! latter corrects the production term near a viscous
                 ! wall.
                 ! The BL decay functin of DDES is now computed
                 ...
                 !  DDES, here we come
                 !
                 DDESrd           = (nu + w(i,j,k,itu1))*kar2Inv*dist2Inv/&
                      max(ss,1.e-10_realType)
                 DDESfd           = one - tanh((8.0_realType*DDESrd)**3)
                 DDES_tilda_d = d2wall(i,j,k) - &
                      DDESfd*max(eps,d2wall(i,j,k) - filterDES(i,j,k))
                 dist2Inv         = one/(DDES_tilda_d**2)
