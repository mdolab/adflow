...
!      ******************************************************************
!
       subroutine readXvelocityPlot3D
!
!      ******************************************************************
!      *                                                                *
!      * readXvelocityPlot3D reads the velocity in x-direction from the *
!      * restart file when Plot3D format is used. It checks if          *
!      * x-velocity if present and reads it. If not present it checks   *
!      * if the x-Momentum is present and constructs the velocity from  *
...
!      ******************************************************************
!
       ! Find the index of the x-velocity in the solution file and check
       ! if it is present.

       p  = nVar
       nn = bsearchStrings(cgnsVelX, varNames, p)

       testXVelPresent: if(nn > 0) then

         ! x-Velocity present. Determine the index in the solution file,
       ...
       else testXVelPresent

         ! No x-velocity. Try x-momentum. If that is not present,
         ...
         ! x-Momentum is present. Determine the offset in the solution
         ! file and read it. Store it at the position of the x-velocity.
         ...
         domainLoop: do nn=1,nDom
           ...
           do k=2,kl
             ...
             do j=2,jl
               ...
               do i=2,il
                 ip = i+p
                 w(ip,jp,kp,ivx) = w(ip,jp,kp,ivx)/w(ip,jp,kp,irho)
               enddo
             enddo
           enddo
       ...
       endif testXVelPresent

       end subroutine readXvelocityPlot3D
