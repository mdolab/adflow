...
!      ******************************************************************
!
       subroutine indexHalo1to1(level, commPattern, internal)
!
!      ******************************************************************
!      *                                                                *
!      * indexHalo1to1 exchanges the global cell indicies for the       *
!      * 1 to 1 matching subfaces                                       *
!      *                                                                *
!      ******************************************************************
!
       use block
...
!I don't think that this should be needed... we'll see.
!!$!
!!$!      Interfaces
!!$!
!!$       interface
       ...
       spectralModes: do mm=1,nTimeIntervalsSpectral
         ...
         sends: do i=1,commPattern(level)%nProcSend
           ...
           do j=1,commPattern(level)%nsend(i)
             ...
             sendBuffer(jj) = flowDoms(d1,level,mm)%globalCell(i1,j1,k1)
             jj = jj + 1
...
!!$                                        internal(level)%periodicData)
         ...
         completeRecvs: do i=1,commPattern(level)%nProcRecv
           ...
           do j=1,commPattern(level)%nrecv(ii)
             ...
             jj = jj + 1
             flowDoms(d2,level,mm)%globalCell(i2,j2,k2) = recvBuffer(jj)
