...
&  skadjb
  REAL(KIND=REALTYPE)&
&  , DIMENSION(iibeg:iiend, jjbeg:jjend, 3), INTENT(IN) :: normadj
...
!v       real(kind=realType) :: tauxy, tauxz, tauyz
  ...
  REAL(KIND=REALTYPE), DIMENSION(iibeg:iiend, jjbeg:jjend) :: pp2b, pp1b
  REAL(KIND=REALTYPE), DIMENSION(iibeg:iiend, jjbeg:jjend) :: rho2, rho1
  REAL(KIND=REALTYPE), DIMENSION(iibeg:iiend, jjbeg:jjend, 3) :: xx
  REAL(KIND=REALTYPE), DIMENSION(iibeg:iiend, jjbeg:jjend, 3) :: xxb
  REAL(KIND=REALTYPE), DIMENSION(iibeg:iiend, jjbeg:jjend, 3) :: ss
  REAL(KIND=REALTYPE), DIMENSION(iibeg:iiend, jjbeg:jjend, 3) :: ssb
  ...
  ELSE
    ...
    DO j=j2end,j2beg,-1
      DO i=i2end,i2beg,-1
        ...
        fxb = zc*cmpadjb(2) + cfpadjb(1) - yc*cmpadjb(3)
        zcb = fx*cmpadjb(2) - fy*cmpadjb(1)
        fzb = yc*cmpadjb(1) + cfpadjb(3) - xc*cmpadjb(2)
        CALL POPREAL8(fz)
    ...
    IF (branch .LT. 4) THEN
      IF (branch .LT. 2) THEN
        ...
        ELSE
          xadjb(:, :, :, :) = 0.0
          xadjb(1, iibeg-1:iiend, jjbeg-1:jjend, :) = xxb(:, :, :)
          siadjb(:, :, :, :) = 0.0
...
&            jjbeg:jjend) + pp2b(:, :)
      ...
      ELSE IF (branch .LT. 3) THEN
        xadjb(:, :, :, :) = 0.0
        xadjb(il, iibeg-1:iiend, jjbeg-1:jjend, :) = xxb(:, :, :)
        siadjb(:, :, :, :) = 0.0
...
&          jjbeg:jjend) + pp2b(:, :)
      ...
      ELSE
        xadjb(:, :, :, :) = 0.0
        xadjb(iibeg-1:iiend, 1, jjbeg-1:jjend, :) = xxb(:, :, :)
        sjadjb(:, :, :, :) = 0.0
...
&          :jjend) + pp2b(:, :)
    ...
    ELSE IF (branch .LT. 6) THEN
      IF (branch .LT. 5) THEN
        xadjb(:, :, :, :) = 0.0
        xadjb(iibeg-1:iiend, jl, jjbeg-1:jjend, :) = xxb(:, :, :)
        sjadjb(:, :, :, :) = 0.0
...
&          jjbeg:jjend) + pp2b(:, :)
      ...
      ELSE
        xadjb(:, :, :, :) = 0.0
        xadjb(iibeg-1:iiend, jjbeg-1:jjend, 1, :) = xxb(:, :, :)
        skadjb(:, :, :, :) = 0.0
...
&          jjend, 2) + pp2b(:, :)
    ...
    ELSE
      xadjb(:, :, :, :) = 0.0
      xadjb(iibeg-1:iiend, jjbeg-1:jjend, kl, :) = xxb(:, :, :)
      skadjb(:, :, :, :) = 0.0
