   ...
   !   gradient     of useful results: moment lift cforce drag force
   !                cd cl cmoment
   !   with respect to varying inputs: pointrefadj moment wblock lengthrefadj
   !                lift surfacerefadj cforce alphaadj pts betaadj
   !                drag force cd cl machcoefadj cmoment
   !   RW status of diff variables: pointrefadj:out moment:in-zero
   !                wblock:out lengthrefadj:out lift:in-zero surfacerefadj:out
   !                cforce:in-zero alphaadj:out pts:out betaadj:out
   !                drag:in-zero force:in-zero cd:in-zero cl:in-zero
   !                machcoefadj:out cmoment:in-zero
   ...
   dragdirb(1) = dragdirb(1) + force(1)*dragb
   forceb(2) = forceb(2) + dragdir(2)*dragb
   dragdirb(2) = dragdirb(2) + force(2)*dragb
   forceb(3) = forceb(3) + dragdir(3)*dragb
   dragdirb(3) = dragdirb(3) + force(3)*dragb
   liftdirb = 0.0
   forceb(1) = forceb(1) + liftdir(1)*liftb
   liftdirb(1) = liftdirb(1) + force(1)*liftb
   forceb(2) = forceb(2) + liftdir(2)*liftb
   liftdirb(2) = liftdirb(2) + force(2)*liftb
   forceb(3) = forceb(3) + liftdir(3)*liftb
   liftdirb(3) = liftdirb(3) + force(3)*liftb
   ...
   dragdirb = 0.0
   dragdirb(1) = dragdirb(1) + rotationmatrix(3, 1)*dragdirtmpb(3)
   dragdirb(2) = dragdirb(2) + rotationmatrix(3, 2)*dragdirtmpb(3)
   dragdirb(3) = dragdirb(3) + rotationmatrix(3, 3)*dragdirtmpb(3)
   dragdirtmpb(3) = 0.0
   ...
   liftdirb = 0.0
   liftdirb(1) = liftdirb(1) + rotationmatrix(3, 1)*liftdirtmpb(3)
   liftdirb(2) = liftdirb(2) + rotationmatrix(3, 2)*liftdirtmpb(3)
   liftdirb(3) = liftdirb(3) + rotationmatrix(3, 3)*liftdirtmpb(3)
   liftdirtmpb(3) = 0.0
   ...
   ELSE
   DO kkk=2,1,-1
   wblockb(ib-kkk-1, i+1, j+1, :) = wblockb(ib-kkk-1, i+1, j+&
   &                1, :) + wadjb(kkk, 2, 2, :)
   wadjb(kkk, 2, 2, :) = 0.0
   ...
   &            wadjb(kkk, 2, 2, :)
   wadjb(kkk, 2, 2, :) = 0.0
   wblockb(i, j+1, kkk+1, :) = wblockb(i, j+1, kkk+1, :) + wadjb(&
   &            kkk, 1, 2, :)
   wadjb(kkk, 1, 2, :) = 0.0
   wblockb(i+1, j, kkk+1, :) = wblockb(i+1, j, kkk+1, :) + wadjb(&
   &            kkk, 2, 1, :)
   wadjb(kkk, 2, 1, :) = 0.0
   wblockb(i, j, kkk+1, :) = wblockb(i, j, kkk+1, :) + wadjb(kkk&
   &            , 1, 1, :)
   wadjb(kkk, 1, 1, :) = 0.0
   END DO
   CALL POPREAL8ARRAY(fact, realtype/8)
