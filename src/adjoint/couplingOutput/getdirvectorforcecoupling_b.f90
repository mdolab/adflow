...
!  
!  Differentiation of getdirvectorforcecoupling in reverse (adjoint) mode:
...
!     ******************************************************************
!
SUBROUTINE GETDIRVECTORFORCECOUPLING_B(refdirection, alpha, alphab, beta&
&  , betab, winddirection, winddirectionb, liftindex)
  USE constants
  IMPLICIT NONE
...
!    ( <=> rotate z-axis alpha radians ccw)
    arg1 = -alpha
    CALL VECTORROTATIONFORCECOUPLING(x1, y1, z1, 3, arg1, xbn, ybn, zbn)
    CALL PUSHREAL8(arg1)
...
!    ( <=> rotate z-axis -beta radians ccw)
  ...
  IF (branch .LT. 2) THEN
    IF (branch .LT. 1) THEN
      CALL VECTORROTATIONFORCECOUPLING_B(xw, xwb, yw, ywb, zw, zwb, 2, &
&                                   arg1, arg1b, x1, x1b, y1, y1b, z1, &
&                                   z1b)
      CALL POPREAL8(arg1)
      betab = -arg1b
      CALL VECTORROTATIONFORCECOUPLING_B(x1, x1b, y1, y1b, z1, z1b, 3, &
&                                   arg1, arg1b, xbn, xbnb, ybn, ybnb, &
&                                   zbn, zbnb)
      alphab = -arg1b
    ELSE
      CALL VECTORROTATIONFORCECOUPLING_B(xw, xwb, yw, ywb, zw, zwb, 3, &
&                                   beta, betab, x1, x1b, y1, y1b, z1, &
&                                   z1b)
      CALL VECTORROTATIONFORCECOUPLING_B(x1, x1b, y1, y1b, z1, z1b, 2, &
&                                   alpha, alphab, xbn, xbnb, ybn, ybnb&
&                                   , zbn, zbnb)
    END IF
  ELSE
    alphab = 0.0
    betab = 0.0
  END IF
END SUBROUTINE GETDIRVECTORFORCECOUPLING_B
