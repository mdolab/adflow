...
!   real(kind=realType) :: fwdValue, revValue
...
!               do l = 1,5
!                  flowdomsb(1,1,1)%dw = zero ! This is actually dw seed
!                  flowdomsb(1,1,1)%dw(i, j, k, l)= one
!                  flowdomsb(1,1,1)%w = zero
                 
!                  time1 = mpi_wtime()
!                  call block_res(nn, 1, .False., alpha, beta, liftIndex, .False.)
!                  time = mpi_wtime() - time1

!                  time1 = mpi_wtime()
!                  ! Now call Bwd block_res
!                  call block_res_b(nn, 1, .False., alpha, beta, liftIndex, force, moment)
                 
!                  timeb = mpi_wtime() - time1
!                  !print *,'Reverse value:', flowdomsb(1,1,1)%w(i,j,k,l)
!                  revValue = flowdomsb(1,1,1)%w(i,j,k,l)
                 
!                  ! Set the seeds
!                  flowdomsd(1,1,1)%w = zero ! This is actually w seed
!                  flowdomsd(1,1,1)%w(i, j, k, l)= one
        
!                  ! Now call Bwd block_res
!                  time1 = mpi_wtime()
!                  call block_res_d(nn, 1, .False., alpha, beta, liftIndex, force, moment)
!                  timed = mpi_wtime() - time1
!                  !print *,time, timed, timeb
!                  fwdValue= flowdomsd(1,1,1)%dw(i,j,k,l)
!                  if (abs(revValue-fwdValue) > 1e-12) then
!                     print *,'Error:', i,j,k,l
!                     print *, revValue, fwdValue
!                  end if
!                  !print *, revValue, fwdValue, time, timed, timeb
!               end do
!            end do
!         end do
!      end do
