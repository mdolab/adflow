...
! it is taken as the average of i and i-1,
    voli = half/vol(i, j, k)
    if (addgridvelocities) qs = (sfacei(i, j, k)+sfacei(i-1, j, k))*voli
...
! it is taken as the average of j and j-1,
    voli = half/vol(i, j, k)
    if (addgridvelocities) qs = (sfacej(i, j, k)+sfacej(i, j-1, k))*voli
! compute the normal velocity, where the normal direction
...
! it is taken as the average of k and k-1,
    voli = half/vol(i, j, k)
    if (addgridvelocities) qs = (sfacek(i, j, k)+sfacek(i, j, k-1))*voli
! compute the normal velocity, where the normal direction
