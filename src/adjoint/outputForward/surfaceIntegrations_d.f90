...
&           cavexponent .ne. int(cavexponent))) then
        ...
        temp = one + exp(arg1)
        sensor1d = (pwr1d-pwr1*exp(arg1)*arg1d/temp)/temp
        sensor1 = pwr1/temp
...
! also do the ks-based cpmin computation
        temp = cpmin_rho*(cpmin_family(spectralsol)-cp)
        ks_exponentd = -(exp(temp)*cpmin_rho*cpd)
        ks_exponent = exp(temp)
