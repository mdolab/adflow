...
&   computespeedofsoundsquared_d, allnodalgradients, allnodalgradients_d
    use fluxes_d
...
&   , a24, a25, a31, a32, a33, a34, a35
    real(kind=realtype) :: resmd, a11d, a15d, a21d, a22d, a25d, a31d, &
&   a33d, a35d
    real(kind=realtype) :: a41, a42, a43, a44, a45, a51, a52, a53, a54, &
&   a55, b11, b12, b13, b14, b15
    real(kind=realtype) :: a41d, a44d, a45d, a51d, a52d, a53d, a54d, &
&   a55d, b11d, b12d, b13d, b14d, b15d
    real(kind=realtype) :: b21, b22, b23, b24, b25, b31, b32, b33, b34, &
&   b35
    real(kind=realtype) :: b21d, b22d, b23d, b24d, b25d, b31d, b32d, &
&   b33d, b34d, b35d
    real(kind=realtype) :: b41, b42, b43, b44, b45, b51, b52, b53, b54, &
&   b55
    real(kind=realtype) :: b41d, b42d, b43d, b44d, b45d, b51d, b52d, &
&   b53d, b54d, b55d
...
&               winf(ivz)*winfd(ivz))
              ...
              x1d = k3d*(velxrho**2+velyrho**2+velzrho**2) + k3*(2*&
&               velxrho*velxrhod+2*velyrho*velyrhod+2*velzrho*velzrhod)
...
&             velzrhod)
            ...
            a55d = -(one*resm*resmd)
            a55 = one*((-(resm**2))/2)
            b11d = (gamma(i, j, k)-1)*(a11d*q+a11*qd)/2 + (a12*velxrho*&
&             wd(i, j, k, irho)-a12*velxrhod*w(i, j, k, irho))/w(i, j, k&
&             , irho)**2 + (a13*velyrho*wd(i, j, k, irho)-a13*velyrhod*w&
&             (i, j, k, irho))/w(i, j, k, irho)**2 + (a14*velzrho*wd(i, &
&             j, k, irho)-a14*velzrhod*w(i, j, k, irho))/w(i, j, k, irho&
&             )**2 + a15d*((gamma(i, j, k)-1)*q/2-sos**2) + a15*((gamma(&
&             i, j, k)-1)*qd/2-2*sos*sosd)
...
&             + a15*(1-gamma(i, j, k))*velzrho
            ...
            b21d = (gamma(i, j, k)-1)*(a21d*q+a21*qd)/2 + ((-(a22d*&
&             velxrho)-a22*velxrhod)*w(i, j, k, irho)+a22*velxrho*wd(i, &
&             j, k, irho))/w(i, j, k, irho)**2 + (a23*velyrho*wd(i, j, k&
&             , irho)-a23*velyrhod*w(i, j, k, irho))/w(i, j, k, irho)**2&
&             + (a24*velzrho*wd(i, j, k, irho)-a24*velzrhod*w(i, j, k, &
&             irho))/w(i, j, k, irho)**2 + a25d*((gamma(i, j, k)-1)*q/2-&
&             sos**2) + a25*((gamma(i, j, k)-1)*qd/2-2*sos*sosd)
...
&             /w(i, j, k, irho) + a25*((gamma(i, j, k)-1)*q/2-sos**2)
            b22d = (1-gamma(i, j, k))*(a21d*velxrho+a21*velxrhod) + (&
&             a22d*w(i, j, k, irho)-a22*wd(i, j, k, irho))/w(i, j, k, &
&             irho)**2 + (1-gamma(i, j, k))*(a25d*velxrho+a25*velxrhod)
            b22 = a21*(1-gamma(i, j, k))*velxrho + a22/w(i, j, k, irho) &
&             + a25*(1-gamma(i, j, k))*velxrho
            b23d = (1-gamma(i, j, k))*(a21d*velyrho+a21*velyrhod) - a23*&
&             wd(i, j, k, irho)/w(i, j, k, irho)**2 + (1-gamma(i, j, k))&
&             *(a25d*velyrho+a25*velyrhod)
            b23 = a21*(1-gamma(i, j, k))*velyrho + a23*1/w(i, j, k, irho&
&             ) + a25*(1-gamma(i, j, k))*velyrho
            b24d = (1-gamma(i, j, k))*(a21d*velzrho+a21*velzrhod) - a24*&
&             wd(i, j, k, irho)/w(i, j, k, irho)**2 + (1-gamma(i, j, k))&
...
&             ) + a25*(1-gamma(i, j, k))*velzrho
            ...
            b31d = (gamma(i, j, k)-1)*(a31d*q+a31*qd)/2 + (a32*velxrho*&
&             wd(i, j, k, irho)-a32*velxrhod*w(i, j, k, irho))/w(i, j, k&
&             , irho)**2 + ((-(a33d*velyrho)-a33*velyrhod)*w(i, j, k, &
&             irho)+a33*velyrho*wd(i, j, k, irho))/w(i, j, k, irho)**2 +&
&             (a34*velzrho*wd(i, j, k, irho)-a34*velzrhod*w(i, j, k, &
&             irho))/w(i, j, k, irho)**2 + a35d*((gamma(i, j, k)-1)*q/2-&
&             sos**2) + a35*((gamma(i, j, k)-1)*qd/2-2*sos*sosd)
...
&             /w(i, j, k, irho) + a35*((gamma(i, j, k)-1)*q/2-sos**2)
            b32d = (1-gamma(i, j, k))*(a31d*velxrho+a31*velxrhod) - a32*&
&             wd(i, j, k, irho)/w(i, j, k, irho)**2 + (1-gamma(i, j, k))&
&             *(a35d*velxrho+a35*velxrhod)
            b32 = a31*(1-gamma(i, j, k))*velxrho + a32/w(i, j, k, irho) &
&             + a35*(1-gamma(i, j, k))*velxrho
            b33d = (1-gamma(i, j, k))*(a31d*velyrho+a31*velyrhod) + (&
&             a33d*w(i, j, k, irho)-a33*wd(i, j, k, irho))/w(i, j, k, &
&             irho)**2 + (1-gamma(i, j, k))*(a35d*velyrho+a35*velyrhod)
            b33 = a31*(1-gamma(i, j, k))*velyrho + a33*1/w(i, j, k, irho&
&             ) + a35*(1-gamma(i, j, k))*velyrho
            b34d = (1-gamma(i, j, k))*(a31d*velzrho+a31*velzrhod) - a34*&
&             wd(i, j, k, irho)/w(i, j, k, irho)**2 + (1-gamma(i, j, k))&
...
&             ) + a35*(1-gamma(i, j, k))*velzrho
            ...
            b41d = (gamma(i, j, k)-1)*(a41d*q+a41*qd)/2 + (a42*velxrho*&
&             wd(i, j, k, irho)-a42*velxrhod*w(i, j, k, irho))/w(i, j, k&
&             , irho)**2 + (a43*velyrho*wd(i, j, k, irho)-a43*velyrhod*w&
&             (i, j, k, irho))/w(i, j, k, irho)**2 + ((-(a44d*velzrho)-&
&             a44*velzrhod)*w(i, j, k, irho)+a44*velzrho*wd(i, j, k, &
&             irho))/w(i, j, k, irho)**2 + a45d*((gamma(i, j, k)-1)*q/2-&
&             sos**2) + a45*((gamma(i, j, k)-1)*qd/2-2*sos*sosd)
...
&             /w(i, j, k, irho) + a45*((gamma(i, j, k)-1)*q/2-sos**2)
            b42d = (1-gamma(i, j, k))*(a41d*velxrho+a41*velxrhod) - a42*&
&             wd(i, j, k, irho)/w(i, j, k, irho)**2 + (1-gamma(i, j, k))&
&             *(a45d*velxrho+a45*velxrhod)
            b42 = a41*(1-gamma(i, j, k))*velxrho + a42/w(i, j, k, irho) &
&             + a45*(1-gamma(i, j, k))*velxrho
            b43d = (1-gamma(i, j, k))*(a41d*velyrho+a41*velyrhod) - a43*&
&             wd(i, j, k, irho)/w(i, j, k, irho)**2 + (1-gamma(i, j, k))&
&             *(a45d*velyrho+a45*velyrhod)
            b43 = a41*(1-gamma(i, j, k))*velyrho + a43*1/w(i, j, k, irho&
&             ) + a45*(1-gamma(i, j, k))*velyrho
            b44d = (1-gamma(i, j, k))*(a41d*velzrho+a41*velzrhod) + (&
&             a44d*w(i, j, k, irho)-a44*wd(i, j, k, irho))/w(i, j, k, &
&             irho)**2 + (1-gamma(i, j, k))*(a45d*velzrho+a45*velzrhod)
...
&             ) + a45*(1-gamma(i, j, k))*velzrho
            ...
            b51d = (gamma(i, j, k)-1)*(a51d*q+a51*qd)/2 + ((-(a52d*&
&             velxrho)-a52*velxrhod)*w(i, j, k, irho)+a52*velxrho*wd(i, &
&             j, k, irho))/w(i, j, k, irho)**2 + ((-(a53d*velyrho)-a53*&
&             velyrhod)*w(i, j, k, irho)+a53*velyrho*wd(i, j, k, irho))/&
&             w(i, j, k, irho)**2 + ((-(a54d*velzrho)-a54*velzrhod)*w(i&
&             , j, k, irho)+a54*velzrho*wd(i, j, k, irho))/w(i, j, k, &
&             irho)**2 + a55d*((gamma(i, j, k)-1)*q/2-sos**2) + a55*((&
...
&             /w(i, j, k, irho) + a55*((gamma(i, j, k)-1)*q/2-sos**2)
            b52d = (1-gamma(i, j, k))*(a51d*velxrho+a51*velxrhod) + (&
&             a52d*w(i, j, k, irho)-a52*wd(i, j, k, irho))/w(i, j, k, &
&             irho)**2 + (1-gamma(i, j, k))*(a55d*velxrho+a55*velxrhod)
            b52 = a51*(1-gamma(i, j, k))*velxrho + a52/w(i, j, k, irho) &
&             + a55*(1-gamma(i, j, k))*velxrho
            b53d = (1-gamma(i, j, k))*(a51d*velyrho+a51*velyrhod) + (&
&             a53d*w(i, j, k, irho)-a53*wd(i, j, k, irho))/w(i, j, k, &
&             irho)**2 + (1-gamma(i, j, k))*(a55d*velyrho+a55*velyrhod)
            b53 = a51*(1-gamma(i, j, k))*velyrho + a53*1/w(i, j, k, irho&
&             ) + a55*(1-gamma(i, j, k))*velyrho
            b54d = (1-gamma(i, j, k))*(a51d*velzrho+a51*velzrhod) + (&
&             a54d*w(i, j, k, irho)-a54*wd(i, j, k, irho))/w(i, j, k, &
&             irho)**2 + (1-gamma(i, j, k))*(a55d*velzrho+a55*velzrhod)
...
&             b12*dwod(2) + b13d*dwo(3) + b13*dwod(3) + b14d*dwo(4) + &
&             b14*dwod(4) + b15d*dwo(5) + b15*dwod(5)
            dw(i, j, k, 1) = b11*dwo(1) + b12*dwo(2) + b13*dwo(3) + b14*&
&             dwo(4) + b15*dwo(5)
            dwd(i, j, k, 2) = b21d*dwo(1) + b21*dwod(1) + b22d*dwo(2) + &
&             b22*dwod(2) + b23d*dwo(3) + b23*dwod(3) + b24d*dwo(4) + &
&             b24*dwod(4) + b25d*dwo(5) + b25*dwod(5)
            dw(i, j, k, 2) = b21*dwo(1) + b22*dwo(2) + b23*dwo(3) + b24*&
&             dwo(4) + b25*dwo(5)
            dwd(i, j, k, 3) = b31d*dwo(1) + b31*dwod(1) + b32d*dwo(2) + &
&             b32*dwod(2) + b33d*dwo(3) + b33*dwod(3) + b34d*dwo(4) + &
&             b34*dwod(4) + b35d*dwo(5) + b35*dwod(5)
            dw(i, j, k, 3) = b31*dwo(1) + b32*dwo(2) + b33*dwo(3) + b34*&
&             dwo(4) + b35*dwo(5)
            dwd(i, j, k, 4) = b41d*dwo(1) + b41*dwod(1) + b42d*dwo(2) + &
&             b42*dwod(2) + b43d*dwo(3) + b43*dwod(3) + b44d*dwo(4) + &
&             b44*dwod(4) + b45d*dwo(5) + b45*dwod(5)
            dw(i, j, k, 4) = b41*dwo(1) + b42*dwo(2) + b43*dwo(3) + b44*&
&             dwo(4) + b45*dwo(5)
            dwd(i, j, k, 5) = b51d*dwo(1) + b51*dwod(1) + b52d*dwo(2) + &
&             b52*dwod(2) + b53d*dwo(3) + b53*dwod(3) + b54d*dwo(4) + &
&             b54*dwod(4) + b55d*dwo(5) + b55*dwod(5)
