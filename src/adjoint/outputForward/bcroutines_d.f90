...
module bcroutines_d
  use constants
  use utils_d, only : terminate, getcorrectfork
  implicit none
  save 
...
!                *rev *p *s *w *rlv *x *si *sj *sk *(*bcdata.norm)
!                *(*bcdata.rface) *(*bcdata.uslip) *xx *rev0 *rev1
...
! ------------------------------------
    do nn=1,nbocos
      if (bctype(nn) .eq. symmpolar) then
        ...
        call bcsymmpolar_d(nn, .false.)
        if (secondhalo) call bcsymmpolar_d(nn, secondhalo)
...
! ------------------------------------
    do nn=1,nbocos
      if (bctype(nn) .eq. symmpolar) then
        ...
        if (secondhalo) call bcsymmpolar(nn, secondhalo)
        call resetbcpointers(nn, .true.)
      end if
    end do
! ------------------------------------
...
! internal cell.
        tmpd = two*(ww2d(i, j, ivx)*nnx+ww2(i, j, ivx)*nnxd+ww2d(i, j, &
&         ivy)*nny+ww2(i, j, ivy)*nnyd+ww2d(i, j, ivz)*nnz+ww2(i, j, ivz&
&         )*nnzd)
...
! determine twice the tangential velocity vector of the
! internal cell.
        tmpd = two*(ww3d(i, j, ivx)*nnx+ww3(i, j, ivx)*nnxd+ww3d(i, j, &
&         ivy)*nny+ww3(i, j, ivy)*nnyd+ww3d(i, j, ivz)*nnz+ww3(i, j, ivz&
&         )*nnzd)
...
!   variations   of useful results: *rev0 *rev1 *pp0 *pp1 *rlv0
!                *rlv1 *ww0 *ww1
!   with respect to varying inputs: rgas *(*bcdata.norm) *rev0
