...
!            murefAdj, timerefAdj)
       subroutine referenceStateForceCouplingAdj(Machadj, MachCoefAdj,uInfAdj,prefAdj,&
            rhorefAdj, pinfdimAdj, rhoinfdimAdj, rhoinfAdj, pinfAdj,&
            murefAdj, timerefAdj)
!
!      ******************************************************************
...
!
       ...
       pInfDimAdj = pRefAdj
       if(pRefAdj <= zero) pInfDimAdj = 101325.0_realType
       ...
       flowTypeTest: if(flowType == internalFlow) then
          ...
          print *,'internal flow not yet Implemented'
          stop
...
!!$         endif
         ...
         if(pRefAdj   <= zero) pRefAdj   = pInfDimAdj
         if(rhoRefAdj <= zero) rhoRefAdj = rhoInfDimAdj
       ...
       muRefAdj = sqrt(pRefAdj*rhoRefAdj)
       ...
       timeRef = sqrt(rhoRefAdj/pRefAdj)
       ...
       uInfAdj   = MachAdj*sqrt(gammaInf*pInfAdj/rhoInfAdj)
       RGas   = RGasDim*rhoRefAdj*TRef/pRefAdj
       muInf  = muDim/muRefAdj
...
!!$
     end subroutine referenceStateForceCouplingAdj
!!$
!!$       !=================================================================
!!$
