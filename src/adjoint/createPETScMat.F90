...
subroutine drdwPCPreAllocation_visc(onProc,offProc,wSize)
  ...
  integer(kind=intType) :: nn,i,j,k,sps,ii,iii,jjj,kkk,i_stencil
  ii = 0
  onProc(:) = 1+(nTimeIntervalsSpectral) ! ALWAYS have the center cell ON-PROCESSOR
  offProc(:) = 0_intType 
        ...
        do k=2,kl
           do j=2,jl
              do i=2,il 
                 ...
                 do i_stencil=1,N_rans_pc
                    iii = rans_pc_stencil(i_stencil,1)
                    jjj = rans_pc_stencil(i_stencil,2)
                    kkk = rans_pc_stencil(i_stencil,3)

                    if (i+iii < 2) then 
                       call checkCell(iMin,j,k,onProc(ii),offProc(ii),1)
                    else if (i+iii > il) then 
                       call checkCell(iMax,j,k,onProc(ii),offProc(ii),1)
                    else if (j+jjj < 2) then 
                       call checkCell(jMin,i,k,onProc(ii),offProc(ii),1)
                    else if (j+jjj > jl) then 
                       call checkCell(jMax,i,k,onProc(ii),offProc(ii),1)
                    else if (k+kkk < 2) then 
                       call checkCell(kMin,i,j,onProc(ii),offProc(ii),1)
                    else if (k+kkk > kl) then 
                       call checkCell(kMax,i,j,onProc(ii),offProc(ii),1)
                    else
                       onProc(ii) = onProc(ii) + 1
                    end if
...
end subroutine drdwPCPreAllocation_visc
