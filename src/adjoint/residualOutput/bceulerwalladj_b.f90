...
!      ******************************************************************
!
SUBROUTINE BCEULERWALLADJ_B(secondhalo, wadj, wadjb, padj, padjb, siadj&
&  , siadjb, sjadj, sjadjb, skadj, skadjb, normadj, normadjb, icell, &
&  jcell, kcell)
      ...
      ELSE
        ...
        DO k=ad_to3,ad_from3,-1
          ...
          DO j=ad_to4,ad_from4,-1
            ...
            padj1b(jj, kk) = 0.0
            CALL POPREAL8(qk)
            skxab = normadj(nn, jj, kk, 1)*rkb + ux*qkb
            skyab = normadj(nn, jj, kk, 2)*rkb + uy*qkb
            skzab = normadj(nn, jj, kk, 3)*rkb + uz*qkb
            CALL POPREAL8(qj)
            sjxab = normadj(nn, jj, kk, 1)*rjb + ux*qjb
            sjyab = normadj(nn, jj, kk, 2)*rjb + uy*qjb
            sjzab = normadj(nn, jj, kk, 3)*rjb + uz*qjb
            CALL POPREAL8(uz)
