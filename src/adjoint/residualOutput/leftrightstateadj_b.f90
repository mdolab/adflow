   ...
   USE precision
   IMPLICIT NONE
   REAL(KIND=REALTYPE), DIMENSION(*), INTENT(IN) :: du1
   REAL(KIND=REALTYPE), DIMENSION(*), INTENT(IN) :: du2
   REAL(KIND=REALTYPE) :: du1b(*), du2b(*), du3b(*)
   REAL(KIND=REALTYPE), DIMENSION(*), INTENT(IN) :: du3
   REAL(KIND=REALTYPE) :: factminmod, omk, opk
   LOGICAL :: firstorderk
   REAL(KIND=REALTYPE) :: left(*), leftb(*), right(*), rightb(*)
   INTEGER(KIND=INTTYPE) :: nwint
   REAL(KIND=REALTYPE), PARAMETER :: epslim=1.e-10_realType
   INTEGER :: branch
   INTEGER(KIND=INTTYPE) :: l
   REAL(KIND=REALTYPE) :: rl1, rl1b, rl2, rl2b, rr1, rr1b, rr2, rr2b, &
   &  temp2b, temp2b0, temp2b1, temp2b2, tmp, tmpb
   REAL(KIND=REALTYPE) :: max1, max1b, max2, max2b, max3, max3b, max4, &
   &  max4b, max5, max5b, max6, max6b, temp, temp0, temp0b, temp1, temp1b, &
   &  temp2, temp2b3, temp3, temp3b, temp4, temp4b, tempb, x1, x1b, x2, x2b&
   &  , x3, x3b, x4, x4b, x5, x5b, x6, x6b
   REAL(KIND=REALTYPE) :: y1, y1b, y2, y2b, y3, y3b, y4, y4b
   INTRINSIC MAX, SIGN, ABS, MIN
   !
   !        ****************************************************************
   !        *                                                              *
