...
!      ******************************************************************
!
subroutine bcSymmAdjTS(wAdj,pAdj,normAdj,iCell,jCell,kCell,secondHalo,nnn,level,sps)
  !
  !      ******************************************************************
  !      *                                                                *
  ...
  !
  logical:: secondHalo
  integer(kind=intType)::nnn,level,sps
...
!!$  nHalo: do mm=0,kk
     ...
     bocos: do nn=1,nBocos
        ...
        call checkOverlapAdjts(nn,icell,jcell,kcell,isbeg,jsbeg,&
...
!!$                                rev1, rev2, mm)
              ...
              call extractBCStatesAdjTS(nn,wAdj,pAdj,wAdj0, wAdj1, wAdj2,wAdj3,&
             ...
             ! stop
              do j=jcBeg,jcEnd
                 do i=icBeg,icEnd
                    ...
                    nnx = normAdj(nn,ii,jj,1,sps)!BCData(nn)%norm(i,j,1)
                    nny = normAdj(nn,ii,jj,2,sps)!BCData(nn)%norm(i,j,2)
                    nnz = normAdj(nn,ii,jj,3,sps)!BCData(nn)%norm(i,j,3)
...
!!$                       if( eddyModel ) revAdj0(ii,jj) = revAdj3(ii,jj)
              ...
              call replaceBCStatesAdjTS(nn,  wAdj0,wAdj1, wAdj2, wAdj3,&
                   pAdj0,pAdj1, pAdj2, pAdj3,rlvAdj1, rlvAdj2,revAdj1, revAdj2,&
                   iCell, jCell,kCell,&
                   wAdj,pAdj,rlvAdj,revAdj,secondHalo,nnn,level,sps)
