...
!      ******************************************************************
!
       subroutine initresAdjTS(varStart, varEnd,wAdj,volAdj,dwAdj,nn,level,sps)
!
!      ******************************************************************
!      *                                                                *
...
!
       ...
       real(kind=realType),dimension(nw,nTimeIntervalsSpectral):: wAdj
...
!
       ...
       real(kind=realType), dimension(nw)::  wspAdj
...
!!$                 enddo
            ...
            elseif(equationMode==Unsteady)then
               !case (unsteady)
               print *,'unsteady not implemented'
...
!!$                 enddo
                 ...
                 timeLoopFine: do mm=1,nTimeIntervalsSpectral
                   ...
                   varLoopFine: do l=varStart,varEnd
                      ...
                      if(l == ivx .or. l == ivy .or. l == ivz) then
                         ...
                         tmp = dvector(jj,ll,ii+1)*wspAdj(ivx) &
                              + dvector(jj,ll,ii+2)*wspAdj(ivy) &
                              + dvector(jj,ll,ii+3)*wspAdj(ivz)
                ...
                enddo timeLoopFine
                  ...
                  call terminate("initRes", "Coarse levels not supported in ADjoint...")
!!$                 ! Coarse grid level. Initialize the owned cells to the
!!$                 ! residual forcing term plus a correction for the
!!$                 ! multigrid treatment of the time derivative term.
!!$
