...
!
       ...
       subroutine metricAdjTS(xAdj,siAdj,sjAdj,skAdj,volAdj,normAdj, &
...
!      ******************************************************************
       ...
       siAdj(:,:,:,:,sps) = zero; sjAdj(:,:,:,:,sps) = zero; skAdj(:,:,:,:,sps) = zero
       normAdj(:,:,:,:,sps) = zero
!!$       do n = 1,3
!!$          do i = -3,2
!!$             do j = -3,2
...
!
       ...
       call volpym4(xAdj(i,j,k,1,sps), xAdj(i,j,k,2,sps), xAdj(i,j,k,3,sps), &
       ...
       call volpym4(xAdj(l,j,k,1,sps), xAdj(l,j,k,2,sps), xAdj(l,j,k,3,sps), &
       ...
       call volpym4(xAdj(i,m,k,1,sps), xAdj(i,m,k,2,sps), xAdj(i,m,k,3,sps), &
            xAdj(i,m,n,1,sps), xAdj(i,m,n,2,sps), xAdj(i,m,n,3,sps), &
            xAdj(l,m,n,1,sps), xAdj(l,m,n,2,sps), xAdj(l,m,n,3,sps), &
       ...
       call volpym4(xAdj(i,j,n,1,sps), xAdj(i,j,n,2,sps), xAdj(i,j,n,3,sps), &
            xAdj(l,j,n,1,sps), xAdj(l,j,n,2,sps), xAdj(l,j,n,3,sps), &
            xAdj(l,m,n,1,sps), xAdj(l,m,n,2,sps), xAdj(l,m,n,3,sps), &
           ...
           bocoLoop: do mm=1,nBocos
              ...
              checkOverlap: if(iOverlap .and. jOverlap .and. kOverlap) then
                 ...
                 case (iMin)
                    ...
                    if(secondHalo)then
                       mult = -one; ss(iSt:iEn,jSt:jEn,:) = siAdj(-1,iSt:iEn,jSt:jEn,:,sps)
                    else 
                       mult = -one; ss(iSt:iEn,jSt:jEn,:) = siAdj(-2,iSt:iEn,jSt:jEn,:,sps)                       
                    end if
                 ...
                 case (iMax)
                    ...
                    if(iRBeg == iREnd) secondHalo = .false.
                    if(secondHalo) then
                       mult = one;  ss(iSt:iEn,jSt:jEn,:) = siAdj(0,iSt:iEn,jSt:jEn,:,sps)
                    else
                       mult = one;  ss(iSt:iEn,jSt:jEn,:) = siAdj(1,iSt:iEn,jSt:jEn,:,sps)
                    end if
                 ...
                 case (jMin)
                   ...
                   if(secondHalo) then
                       mult = -one; ss(iSt:iEn,jSt:jEn,:) = sjAdj(iSt:iEn,-1,jSt:jEn,:,sps)
                    else
                       mult = -one; ss(iSt:iEn,jSt:jEn,:) = sjAdj(iSt:iEn,-2,jSt:jEn,:,sps)
                    end if                    
                 ...
                 case (jMax)
                    ...
                    if(jRBeg == jREnd) secondHalo = .false.
                   if(secondHalo) then
                       mult = one;  ss(iSt:iEn,jSt:jEn,:) = sjAdj(iSt:iEn,0,jSt:jEn,:,sps)
                    else
                       mult = one;  ss(iSt:iEn,jSt:jEn,:) = sjAdj(iSt:iEn,1,jSt:jEn,:,sps)
                    end if                    
                 ...
                 case (kMin)
                    ...
                    if(secondHalo) then
                       mult = -one; ss(iSt:iEn,jSt:jEn,:) = skAdj(iSt:iEn,jSt:jEn,-1,:,sps)
                    else
                       mult = -one; ss(iSt:iEn,jSt:jEn,:) = skAdj(iSt:iEn,jSt:jEn,-2,:,sps)
                    end if
                 ...
                 case (kMax)
                    ...
                    if(kRBeg == kREnd) secondHalo = .false.
                    if(secondHalo) then
                       mult = one;  ss(iSt:iEn,jSt:jEn,:) = skAdj(iSt:iEn,jSt:jEn,0,:,sps)
                    else
                       mult = one;  ss(iSt:iEn,jSt:jEn,:) = skAdj(iSt:iEn,jSt:jEn,1,:,sps)
                    end if                    
                 end select
...
!!$                       normAdj(mm,jj,kk,3) = fact*zp
                       ...
                       if( xp**2>zero .or. yp**2>zero .or. zp**2>zero)then
                          ...
                          fact = mult/fact
                          !compute unit normal...
                          normAdj(mm,jj,kk,1,sps) = fact*xp
                          normAdj(mm,jj,kk,2,sps) = fact*yp
                          normAdj(mm,jj,kk,3,sps) = fact*zp
                       else
                          !Length is zero
                          normAdj(mm,jj,kk,:,sps) = zero
                       endif
                    enddo
                 enddo
...
!
           debugging: if( debug ) then
              ...
              v1(1) = siAdj(i,j,k,1,sps) + sjAdj(i,j,k,1,sps) + skAdj(i,j,k,1,sps) &
                    - siAdj(l,j,k,1,sps) - sjAdj(i,m,k,1,sps) - skAdj(i,j,n,1,sps)
              v1(2) = siAdj(i,j,k,2,sps) + sjAdj(i,j,k,2,sps) + skAdj(i,j,k,2,sps) &
                    - siAdj(l,j,k,2,sps) - sjAdj(i,m,k,2,sps) - skAdj(i,j,n,2,sps)
              v1(3) = siAdj(i,j,k,3,sps) + sjAdj(i,j,k,3,sps) + skAdj(i,j,k,3,sps) &
                    - siAdj(l,j,k,3,sps) - sjAdj(i,m,k,3,sps) - skAdj(i,j,n,3,sps)
              
              ! Store the inverse of the sum of the areas of the
              ! six faces in fact.

              fact = one/(sqrt(siAdj(i,j,k,1,sps)*siAdj(i,j,k,1,sps)  &
                   +           siAdj(i,j,k,2,sps)*siAdj(i,j,k,2,sps)  &
                   +           siAdj(i,j,k,3,sps)*siAdj(i,j,k,3,sps)) &
                   +      sqrt(siAdj(l,j,k,1,sps)*siAdj(l,j,k,1,sps)  &
                   +           siAdj(l,j,k,2,sps)*siAdj(l,j,k,2,sps)  &
                   +           siAdj(l,j,k,3,sps)*siAdj(l,j,k,3,sps)) &
                   +      sqrt(sjAdj(i,j,k,1,sps)*sjAdj(i,j,k,1,sps)  &
                   +           sjAdj(i,j,k,2,sps)*sjAdj(i,j,k,2,sps)  &
                   +           sjAdj(i,j,k,3,sps)*sjAdj(i,j,k,3,sps)) &
                   +      sqrt(sjAdj(i,m,k,1,sps)*sjAdj(i,m,k,1,sps)  &
                   +           sjAdj(i,m,k,2,sps)*sjAdj(i,m,k,2,sps)  &
                   +           sjAdj(i,m,k,3,sps)*sjAdj(i,m,k,3,sps)) &
                   +      sqrt(skAdj(i,j,k,1,sps)*skAdj(i,j,k,1,sps)  &
                   +           skAdj(i,j,k,2,sps)*skAdj(i,j,k,2,sps)  &
                   +           skAdj(i,j,k,3,sps)*skAdj(i,j,k,3,sps)) &
                   +      sqrt(skAdj(i,j,n,1,sps)*skAdj(i,j,n,1,sps)  &
                   +           skAdj(i,j,n,2,sps)*skAdj(i,j,n,2,sps)  &
                   +           skAdj(i,j,n,3,sps)*skAdj(i,j,n,3,sps)))
...
end subroutine metricAdjTS


!      ==================================================================
!      ================================================================

  subroutine volpym4(xa,ya,za,xb,yb,zb,xc,yc,zc,xd,yd,zd,xp,yp,zp,vp)
!
!        ****************************************************************
!        *                                                              *
