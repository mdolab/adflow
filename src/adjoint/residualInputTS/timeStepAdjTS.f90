...
!      ******************************************************************
!
       subroutine timeStepAdjTS(onlyRadii,wAdj,pAdj,siAdj, sjAdj, skAdj,&
                ...
                do k=-1,1
                 do j=-1,1
                   do i=-1,1
                     ...
                     cc2 = gamma(ii,jj,kk)*pAdj(i,j,k,sps)/wAdj(i,j,k,irho,sps)
                     cc2 = max(cc2,clim2)
                     ...
                     dtlAdj = radiAdj(i,j,k,sps) + radJAdj(i,j,k,sps) + radKAdj(i,j,k,sps)
...
!!$                 do i=1,ie
                   ...
                   ri = max(radiAdj(i,j,k,sps),eps)
                   rj = max(radJAdj(i,j,k,sps),eps)
                   rk = max(radKAdj(i,j,k,sps),eps)
               ...
               !   radK(i,j,k) = third*radK(i,j,k)*(one + one/rki + rjk)
                   ...
                   radiAdj(i,j,k,sps) = radiAdj(i,j,k,sps)*(one + one/rij + rki)
                   radJAdj(i,j,k,sps) = radJAdj(i,j,k,sps)*(one + one/rjk + rij)
                   radKAdj(i,j,k,sps) = radKAdj(i,j,k,sps)*(one + one/rki + rjk)
...
!!$                 do i=2,il
                   ...
                   dpi = abs(pAdj(i+1,j,k,sps) - two*pAdj(i,j,k,sps) + pAdj(i-1,j,k,sps)) &
                       /    (pAdj(i+1,j,k,sps) + two*pAdj(i,j,k,sps) + pAdj(i-1,j,k,sps) + plim)
                   dpj = abs(pAdj(i,j+1,k,sps) - two*pAdj(i,j,k,sps) + pAdj(i,j-1,k,sps)) &
                       /    (pAdj(i,j+1,k,sps) + two*pAdj(i,j,k,sps) + pAdj(i,j-1,k,sps) + plim)
                   dpk = abs(pAdj(i,j,k+1,sps) - two*pAdj(i,j,k,sps) + pAdj(i,j,k-1,sps)) &
                       /    (pAdj(i,j,k+1,sps) + two*pAdj(i,j,k,sps) + pAdj(i,j,k-1,sps) + plim)
