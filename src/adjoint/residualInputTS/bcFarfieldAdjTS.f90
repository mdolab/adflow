...
!      ******************************************************************
!
subroutine bcFarfieldAdjTS(secondHalo,wInfAdj,pInfCorrAdj, wAdj,pAdj,      &
  ...
  !
  integer(kind=intType) ::nn,level,sps! it's not needed anymore w/ normAdj
        ...
        testFarfield: if(BCType(nnbcs) == FarField) then
           ...
           do j=jcBeg, jcEnd
              do i=icBeg, icEnd
                 ...
                 nnx = normAdj(nnbcs,ii,jj,1,sps)
                 nny = normAdj(nnbcs,ii,jj,2,sps)
                 nnz = normAdj(nnbcs,ii,jj,3,sps)
!!$                 if(i==1 .and. j ==1)then
...
!!$                 write(unitxAD,11)int((icbeg+icend)/2),int((jcbeg+jcend)/2), i,j,nnbcs,normAdj(nnbcs,ii,jj,1,sps), rFaceAdj(nnbcs,ii,jj,sps) 
...
!close (UNIT=unitxAD)
end subroutine bcFarfieldAdjTS
