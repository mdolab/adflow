...
!      ******************************************************************
!
       subroutine gridVelocitiesFineLevelAdjTS(useOldCoor, t, sps,xAdj,&
...
!
       integer(kind=intType), intent(in) :: sps,nn,level
...
!
    ...
    !  velzGrid = aInf*MachGrid(3)acg[i][j]
       ...
       call derivativeRotMatrixRigidAdjTS(derivRotationMatrixAdj, rotationPointAdj,rotPointAdj, t(1))
       ...
       if(TSStability)then
          ...
          if(TSpMode.or.TSqMode .or.TSrMode) then
             ...
             call rotMatrixRigidBodyAdjTS(tNew, tOld, rotationMatrixAdj, rotationPointAdj,rotPointAdj)
          ...
          elseif(tsAlphaMode)then
            ...
            !      liftIndex,alpha,beta)
             ...
             call adjustInflowAngleAdjTS(alphaTS,betaAdj,velDir,liftDir,dragDir,&
                  liftIndex)
          ...
          elseif(tsBetaMode)then
             ...
             !Determine the grid velocity for this alpha
             call adjustInflowAngleAdjTS(alphaAdj,betaTS,velDir,liftDir,dragDir,&
                  liftIndex)
             !do I need to update the lift direction and drag direction as well?
             !set the effictive grid velocity for this time interval
             velxGrid0 = (aInf*machgridAdj)*(-velDir(1))
...
!!$                 do i=1,ie
                   ...
                   xc(1) = eighth*(xAdj(i-1,j-1,k-1,1,sps) + xAdj(i,j-1,k-1,1,sps) &
                         +         xAdj(i-1,j,  k-1,1,sps) + xAdj(i,j,  k-1,1,sps) &
                         +         xAdj(i-1,j-1,k,  1,sps) + xAdj(i,j-1,k,  1,sps) &
                         +         xAdj(i-1,j,  k,  1,sps) + xAdj(i,j,  k,  1,sps))
                   xc(2) = eighth*(xAdj(i-1,j-1,k-1,2,sps) + xAdj(i,j-1,k-1,2,sps) &
                         +         xAdj(i-1,j,  k-1,2,sps) + xAdj(i,j,  k-1,2,sps) &
                         +         xAdj(i-1,j-1,k,  2,sps) + xAdj(i,j-1,k,  2,sps) &
                         +         xAdj(i-1,j,  k,  2,sps) + xAdj(i,j,  k,  2,sps))
                   xc(3) = eighth*(xAdj(i-1,j-1,k-1,3,sps) + xAdj(i,j-1,k-1,3,sps) &
                         +         xAdj(i-1,j,  k-1,3,sps) + xAdj(i,j,  k-1,3,sps) &
                         +         xAdj(i-1,j-1,k,  3,sps) + xAdj(i,j-1,k,  3,sps) &
...
!
               ...
               do i=istart,iend
                 ...
                 select case (mm)
                 case (1_intType)       ! normals in i-direction
                    sFaceIAdj(i,:,:,sps) = sFaceAdj
                    
                 case (2_intType)       ! normals in j-direction
                    sFaceJAdj(:,i,:,sps) = sFaceAdj
                    
                 case (3_intType)       ! normals in k-direction
                    sFaceKAdj(:,:,i,sps) = sFaceAdj
                 end select
 ...
 !      enddo domains
       ...
       end subroutine gridVelocitiesFineLevelAdjTS
