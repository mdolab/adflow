   ...
   !  
   !  Differentiation of bceulerwalladjts in reverse (adjoint) mode:
   ...
   !      ******************************************************************
   !
   SUBROUTINE BCEULERWALLADJTS_B(secondhalo, wadj, wadjb, padj, padjb, sadj&
   &  , sadjb, siadj, siadjb, sjadj, sjadjb, skadj, skadjb, normadj, &
   &  normadjb, rfaceadj, rfaceadjb, icell, jcell, kcell, nn, level, sps)
   USE bctypes
   ...
   CALL EXTRACTBCSTATESADJTS(nnbcs, wadj, padj, wadj0, wadj1, wadj2&
   ...
   rxj = a1*(normadj(nnbcs, jjp1, kk, 1, sps)-normadj(nnbcs, &
   &                jjm1, kk, 1, sps))
   CALL PUSHREAL8(ryj)
   ryj = a1*(normadj(nnbcs, jjp1, kk, 2, sps)-normadj(nnbcs, &
   &                jjm1, kk, 2, sps))
   CALL PUSHREAL8(rzj)
   rzj = a1*(normadj(nnbcs, jjp1, kk, 3, sps)-normadj(nnbcs, &
   &                jjm1, kk, 3, sps))
   CALL PUSHREAL8(dpj)
   !print *, "jjp1,jjm1, kk =", jjp1,jjm1, kk
   dpj = a1*(padj2(jjp1, kk)-padj2(jjm1, kk))
   CALL PUSHREAL8(rxk)
   rxk = b1*(normadj(nnbcs, jj, kkp1, 1, sps)-normadj(nnbcs, &
   &                jj, kkm1, 1, sps))
   CALL PUSHREAL8(ryk)
   ryk = b1*(normadj(nnbcs, jj, kkp1, 2, sps)-normadj(nnbcs, &
   &                jj, kkm1, 2, sps))
   CALL PUSHREAL8(rzk)
   rzk = b1*(normadj(nnbcs, jj, kkp1, 3, sps)-normadj(nnbcs, &
   &                jj, kkm1, 3, sps))
   CALL PUSHREAL8(dpk)
   ...
   ri = normadj(nnbcs, jj, kk, 1, sps)*sixa + normadj(nnbcs, &
   &                jj, kk, 2, sps)*siya + normadj(nnbcs, jj, kk, 3, sps)*&
   &                siza
   CALL PUSHREAL8(rj)
   rj = normadj(nnbcs, jj, kk, 1, sps)*sjxa + normadj(nnbcs, &
   &                jj, kk, 2, sps)*sjya + normadj(nnbcs, jj, kk, 3, sps)*&
   &                sjza
   CALL PUSHREAL8(rk)
   rk = normadj(nnbcs, jj, kk, 1, sps)*skxa + normadj(nnbcs, &
   &                jj, kk, 2, sps)*skya + normadj(nnbcs, jj, kk, 3, sps)*&
   &                skza
   CALL PUSHREAL8(ux)
   ...
   jj = j - joffset
   !BCData(nn)%rface(i,j)
   rface = rfaceadj(nnbcs, ii, jj, sps)
   ...
   &              , sps)-wadj2(ii, jj, ivy)*normadj(nnbcs, ii, jj, 2, sps)-&
   ...
   CALL REPLACEBCSTATESADJTS_B(nnbcs, wadj0, wadj0b, wadj1, wadj1b, &
   ...
   CALL EXTRAPOLATE2NDHALOADJTS_B(nnbcs, icbeg, icend, jcbeg, jcend&
   ...
   &            )
   vnb = normadj(nnbcs, ii, jj, 3, sps)*wadj1b(ii, jj, ivz)
   normadjb(nnbcs, ii, jj, 3, sps) = normadjb(nnbcs, ii, jj, 3, &
   &            sps) + vn*wadj1b(ii, jj, ivz)
   wadj1b(ii, jj, ivz) = 0.0
   CALL POPREAL8(wadj1(ii, jj, ivy))
   wadj2b(ii, jj, ivy) = wadj2b(ii, jj, ivy) + wadj1b(ii, jj, ivy&
   &            )
   vnb = vnb + normadj(nnbcs, ii, jj, 2, sps)*wadj1b(ii, jj, ivy)
   normadjb(nnbcs, ii, jj, 2, sps) = normadjb(nnbcs, ii, jj, 2, &
   &            sps) + vn*wadj1b(ii, jj, ivy)
   wadj1b(ii, jj, ivy) = 0.0
   CALL POPREAL8(wadj1(ii, jj, ivx))
   wadj2b(ii, jj, ivx) = wadj2b(ii, jj, ivx) + wadj1b(ii, jj, ivx&
   &            )
   vnb = vnb + normadj(nnbcs, ii, jj, 1, sps)*wadj1b(ii, jj, ivx)
   temp2b0 = two*vnb
   normadjb(nnbcs, ii, jj, 1, sps) = normadjb(nnbcs, ii, jj, 1, &
   &            sps) + vn*wadj1b(ii, jj, ivx) - wadj2(ii, jj, ivx)*temp2b0
   ...
   CALL POPREAL8(rk)
   normadjb(nnbcs, jj, kk, 1, sps) = normadjb(nnbcs, jj, kk, 1&
   &              , sps) + skxa*rkb
   skxab = skxab + normadj(nnbcs, jj, kk, 1, sps)*rkb
   normadjb(nnbcs, jj, kk, 2, sps) = normadjb(nnbcs, jj, kk, 2&
   &              , sps) + skya*rkb
   skyab = skyab + normadj(nnbcs, jj, kk, 2, sps)*rkb
   normadjb(nnbcs, jj, kk, 3, sps) = normadjb(nnbcs, jj, kk, 3&
   &              , sps) + skza*rkb
   skzab = skzab + normadj(nnbcs, jj, kk, 3, sps)*rkb
   CALL POPREAL8(rj)
   normadjb(nnbcs, jj, kk, 1, sps) = normadjb(nnbcs, jj, kk, 1&
   &              , sps) + sjxa*rjb
   sjxab = sjxab + normadj(nnbcs, jj, kk, 1, sps)*rjb
   normadjb(nnbcs, jj, kk, 2, sps) = normadjb(nnbcs, jj, kk, 2&
   &              , sps) + sjya*rjb
   sjyab = sjyab + normadj(nnbcs, jj, kk, 2, sps)*rjb
   normadjb(nnbcs, jj, kk, 3, sps) = normadjb(nnbcs, jj, kk, 3&
   &              , sps) + sjza*rjb
   sjzab = sjzab + normadj(nnbcs, jj, kk, 3, sps)*rjb
   CALL POPREAL8(ri)
   normadjb(nnbcs, jj, kk, 1, sps) = normadjb(nnbcs, jj, kk, 1&
   &              , sps) + sixa*rib
   sixab = normadj(nnbcs, jj, kk, 1, sps)*rib
   normadjb(nnbcs, jj, kk, 2, sps) = normadjb(nnbcs, jj, kk, 2&
   &              , sps) + siya*rib
   siyab = normadj(nnbcs, jj, kk, 2, sps)*rib
   normadjb(nnbcs, jj, kk, 3, sps) = normadjb(nnbcs, jj, kk, 3&
   &              , sps) + siza*rib
   sizab = normadj(nnbcs, jj, kk, 3, sps)*rib
   CALL POPREAL8(dpk)
   padj2b(jj, kkp1) = padj2b(jj, kkp1) + b1*dpkb
   padj2b(jj, kkm1) = padj2b(jj, kkm1) - b1*dpkb
   CALL POPREAL8(rzk)
   normadjb(nnbcs, jj, kkp1, 3, sps) = normadjb(nnbcs, jj, kkp1&
   &              , 3, sps) + b1*rzkb
   normadjb(nnbcs, jj, kkm1, 3, sps) = normadjb(nnbcs, jj, kkm1&
   &              , 3, sps) - b1*rzkb
   CALL POPREAL8(ryk)
   normadjb(nnbcs, jj, kkp1, 2, sps) = normadjb(nnbcs, jj, kkp1&
   &              , 2, sps) + b1*rykb
   normadjb(nnbcs, jj, kkm1, 2, sps) = normadjb(nnbcs, jj, kkm1&
   &              , 2, sps) - b1*rykb
   CALL POPREAL8(rxk)
   normadjb(nnbcs, jj, kkp1, 1, sps) = normadjb(nnbcs, jj, kkp1&
   &              , 1, sps) + b1*rxkb
   normadjb(nnbcs, jj, kkm1, 1, sps) = normadjb(nnbcs, jj, kkm1&
   &              , 1, sps) - b1*rxkb
   CALL POPREAL8(dpj)
   padj2b(jjp1, kk) = padj2b(jjp1, kk) + a1*dpjb
   padj2b(jjm1, kk) = padj2b(jjm1, kk) - a1*dpjb
   CALL POPREAL8(rzj)
   normadjb(nnbcs, jjp1, kk, 3, sps) = normadjb(nnbcs, jjp1, kk&
   &              , 3, sps) + a1*rzjb
   normadjb(nnbcs, jjm1, kk, 3, sps) = normadjb(nnbcs, jjm1, kk&
   &              , 3, sps) - a1*rzjb
   CALL POPREAL8(ryj)
   normadjb(nnbcs, jjp1, kk, 2, sps) = normadjb(nnbcs, jjp1, kk&
   &              , 2, sps) + a1*ryjb
   normadjb(nnbcs, jjm1, kk, 2, sps) = normadjb(nnbcs, jjm1, kk&
   &              , 2, sps) - a1*ryjb
   CALL POPREAL8(rxj)
   normadjb(nnbcs, jjp1, kk, 1, sps) = normadjb(nnbcs, jjp1, kk&
   &              , 1, sps) + a1*rxjb
   normadjb(nnbcs, jjm1, kk, 1, sps) = normadjb(nnbcs, jjm1, kk&
   &              , 1, sps) - a1*rxjb
   CALL POPREAL8(skza)
   ...
   ELSE IF (branch .LT. 3) THEN
   sadjb(0, :, :, :, sps) = sadjb(0, :, :, :, sps) + ssadjb&
   &                  (:, :, :)
   ssadjb(:, :, :) = 0.0
   ELSE
   GOTO 100
   END IF
   skadjb(0, :, :, :, sps) = skadjb(0, :, :, :, sps) + sskb(:&
   &                , :, :)
   sjadjb(0, :, :, :, sps) = sjadjb(0, :, :, :, sps) + ssjb(:&
   &                , :, :)
   siadjb(-1, :, :, :, sps) = siadjb(-1, :, :, :, sps) + ssib&
   &                (:, :, :)
   ELSE
   IF (branch .LT. 6) THEN
   IF (branch .LT. 5) THEN
   sadjb(-1, :, :, :, sps) = sadjb(-1, :, :, :, sps) + &
   &                    ssadjb(:, :, :)
   ssadjb(:, :, :) = 0.0
   GOTO 100
   END IF
   ...
   sjadjb(0, :, :, :, sps) = sjadjb(0, :, :, :, sps) + ssjb(:&
   &                , :, :)
   siadjb(0, :, :, :, sps) = siadjb(0, :, :, :, sps) + ssib(:&
   &                , :, :)
   END IF
   GOTO 140
   100        skadjb(-1, :, :, :, sps) = skadjb(-1, :, :, :, sps) + sskb(:&
   &              , :, :)
   sjadjb(-1, :, :, :, sps) = sjadjb(-1, :, :, :, sps) + ssjb(:&
   &              , :, :)
   siadjb(-2, :, :, :, sps) = siadjb(-2, :, :, :, sps) + ssib(:&
   &              , :, :)
   ELSE
   ...
   ssadjb(:, :, :) = 0.0
   END IF
   skadjb(1, :, :, :, sps) = skadjb(1, :, :, :, sps) + sskb&
   &                  (:, :, :)
   sjadjb(1, :, :, :, sps) = sjadjb(1, :, :, :, sps) + ssjb&
   &                  (:, :, :)
   siadjb(1, :, :, :, sps) = siadjb(1, :, :, :, sps) + ssib&
   ...
   skadjb(:, -1, :, :, sps) = skadjb(:, -1, :, :, sps) + sskb&
   &                (:, :, :)
   siadjb(:, -1, :, :, sps) = siadjb(:, -1, :, :, sps) + ssjb&
   &                (:, :, :)
   sjadjb(:, -2, :, :, sps) = sjadjb(:, -2, :, :, sps) + ssib&
   &                (:, :, :)
   GOTO 140
   END IF
   110        skadjb(:, 0, :, :, sps) = skadjb(:, 0, :, :, sps) + sskb(:, &
   &              :, :)
   siadjb(:, 0, :, :, sps) = siadjb(:, 0, :, :, sps) + ssjb(:, &
   &              :, :)
   sjadjb(:, -1, :, :, sps) = sjadjb(:, -1, :, :, sps) + ssib(:&
   &              , :, :)
   END IF
   ...
   siadjb(:, 0, :, :, sps) = siadjb(:, 0, :, :, sps) + ssjb(:&
   &                , :, :)
   sjadjb(:, 0, :, :, sps) = sjadjb(:, 0, :, :, sps) + ssib(:&
   &                , :, :)
   GOTO 140
   END IF
   ELSE
   ...
   sadjb(:, 1, :, :, sps) = sadjb(:, 1, :, :, sps) + ssadjb&
   &                  (:, :, :)
   ssadjb(:, :, :) = 0.0
   GOTO 120
   END IF
   ...
   ssadjb(:, :, :) = 0.0
   END IF
   sjadjb(:, :, 0, :, sps) = sjadjb(:, :, 0, :, sps) + sskb(:, &
   &              :, :)
   siadjb(:, :, 0, :, sps) = siadjb(:, :, 0, :, sps) + ssjb(:, &
   &              :, :)
   skadjb(:, :, -1, :, sps) = skadjb(:, :, -1, :, sps) + ssib(:&
   &              , :, :)
   GOTO 140
   END IF
   120      skadjb(:, 1, :, :, sps) = skadjb(:, 1, :, :, sps) + sskb(:, :&
   &            , :)
   siadjb(:, 1, :, :, sps) = siadjb(:, 1, :, :, sps) + ssjb(:, :&
   &            , :)
   sjadjb(:, 1, :, :, sps) = sjadjb(:, 1, :, :, sps) + ssib(:, :&
   ...
   ssadjb(:, :, :) = 0.0
   END IF
   sjadjb(:, :, -1, :, sps) = sjadjb(:, :, -1, :, sps) + sskb&
   &                (:, :, :)
   siadjb(:, :, -1, :, sps) = siadjb(:, :, -1, :, sps) + ssjb&
   &                (:, :, :)
   skadjb(:, :, -2, :, sps) = skadjb(:, :, -2, :, sps) + ssib&
   &                (:, :, :)
   GOTO 140
   END IF
   ELSE
   ...
   GOTO 140
   END IF
   130      sjadjb(:, :, 0, :, sps) = sjadjb(:, :, 0, :, sps) + sskb(:, :&
   &            , :)
   siadjb(:, :, 0, :, sps) = siadjb(:, :, 0, :, sps) + ssjb(:, :&
   &            , :)
   skadjb(:, :, 0, :, sps) = skadjb(:, :, 0, :, sps) + ssib(:, :&
   &            , :)
   END IF
   END IF
   ...
   wadj3b(:, :, :) = 0.0
   CALL EXTRACTBCSTATESADJTS_B(nnbcs, wadj, wadjb, padj, padjb, wadj0&
   ...
   END DO
   END SUBROUTINE BCEULERWALLADJTS_B
