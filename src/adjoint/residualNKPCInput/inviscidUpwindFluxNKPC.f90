...
!      ******************************************************************
!
subroutine inviscidUpwindFluxNKPC(wAdj,  pAdj,  dwAdj, &
     siAdj, sjAdj, skAdj, &
  ...
  orderTest: if(limiter == firstOrder) then
     ...
     do ii=-1,0
        ...
        call riemannFluxNKPC(left, right, flux,por,gammaFace,correctForK,sX,sY,sZ,sFace,fineGrid)
...
!!$           print *,'pAdj',p(i,  j,k) - p(i-1,j,k),pAdj(ii,  0,0) - pAdj(ii-1,0,0),p(i,  j,k),p(i-1,j,k),pAdj(ii,  0,0), pAdj(ii-1,0,0),p(i,  j,k) -pAdj(ii,  0,0),p(i,  j,k) - p(i-1,j,k)-(pAdj(ii,  0,0) - pAdj(ii-1,0,0))
        ...
        call leftRightStateNKPC(du1, du2, du3, left, right,nwInt,omk,opk,factminmod,firstOrderK)
...
end subroutine inviscidUpwindFluxNKPC
