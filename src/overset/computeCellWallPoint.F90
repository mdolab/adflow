subroutine computeCellWallPoint(level, sps)
  ...
  ! Working paramters
  integer(kind=intType) :: i, j, k, nn, cluster
  ...
  do nn=1, nDom
     ...
     if (treeBuilt(cluster) .eqv. .False. .and. wall%nNodes > 0) then 
        wall%tree => kdtree2_create(wall%x(:, 1:wall%nNodes), sort=.False.)
     end if
        ...
        do k=2, kl
           do j=2, jl
              do i=2, il
                 ...
                 xseed(i, j, k, :) = wall%x(:, results(1)%idx)
                 wallInd(i, j, k) = wall%ind(results(1)%idx)
  ...
  ! Loop back over the block destroying the kd_trees as necessary
  do nn=1, nDom
     call setPointers(nn, level, sps)
     cluster = clusters(cumDomProc(myid) + nn)
     wall => clusterWalls(cluster)

     if (treeBuilt(cluster) .eqv. .True.) then 
        call kdtree2_destroy(wall%tree)
     end if
  end do

  ! Finally do a communication just to make sure halos have the same
  ! values. this isn't strictly necessay
  do nn=1, nDom
...
! =============================================================
! Original Communciation based method. It does not work correctly
! when blocks are highly partitioned
! =============================================================
...
!      ! Run the generic halo exchange. Only need to use the first level
!      ! halos.  Pointers are already set from above.
