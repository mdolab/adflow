subroutine exchangeSurfaceDelta(level, sps, commPattern, internal)
  ...
  do nn=1, nDom
     ...
     allocate(flowDoms(nn, level, sps)%rVar1(0:ib, 0:jb, 0:kb))
     ...
     bocoLoop: do mm=1, nBocos
        wallType: if (BCType(mm) == EulerWall .or. &
           ...
           case (iMin)
              deltaPtr => flowDoms(nn, level, sps)%rVar1(2, :, :)
           case (iMax)
              deltaPtr => flowDoms(nn, level, sps)%rVar1(il, :, :)
           case (jMin)
              deltaPtr => flowDoms(nn, level, sps)%rVar1(:, 2, :)
           case (jMax)
              deltaPtr => flowDoms(nn, level, sps)%rVar1(:, jl, :)
           case (kMin)
              deltaPtr => flowDoms(nn, level, sps)%rVar1(:, :, 2)
           case (kMax)
              deltaPtr => flowDoms(nn, level, sps)%rVar1(:, :, kl)
           end select
  ...
  do nn=1, nDom
     ...
     bocoLoop2: do mm=1, nBocos
      ...
      deallocate(flowDoms(nn, level, sps)%rVar1)
   end do
