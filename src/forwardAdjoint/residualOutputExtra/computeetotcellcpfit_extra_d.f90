   ...
   !   with respect to varying inputs: rgas cv0 *cptrange *cpeint
   !                *(*cptempfit.constants) *cptempfit.eint0 cvn *p
   !                *gamma *w scale
   !      ==================================================================
   SUBROUTINE COMPUTEETOTCELLCPFIT_EXTRA_D(i, j, k, scale, scaled, &
   &  correctfork)
   USE FLOWVARREFSTATE
   USE CPCURVEFITS
   USE BLOCKPOINTERS_D
   ...
   &          )-1)*td
   ELSE IF (t .EQ. 0.0 .AND. cptempfit(nn)%exponents(ii) .EQ. 1.0) &
   &      THEN
   t2d = td
   ELSE
   t2d = 0.0
   ...
   &      itu1)
   END IF
   END SUBROUTINE COMPUTEETOTCELLCPFIT_EXTRA_D
