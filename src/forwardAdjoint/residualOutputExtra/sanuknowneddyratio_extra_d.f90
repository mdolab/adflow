   ...
   !
   !  Differentiation of sanuknowneddyratio in forward (tangent) mode:
   ...
   !      ******************************************************************
   !
   FUNCTION SANUKNOWNEDDYRATIO_EXTRA_D(eddyratio, nulam, nulamd, &
   &  sanuknowneddyratio)
   USE PARAMTURB
   USE CONSTANTS
   IMPLICIT NONE
   ...
   sanuknowneddyratio_extra_d = 0.0
   RETURN
   ELSE
   ...
   IF (abs1 .LE. thresholdreal) GOTO 100
   END DO
   ...
   100 sanuknowneddyratio_extra_d = chi*nulamd
   sanuknowneddyratio = nulam*chi
   END IF
   END FUNCTION SANUKNOWNEDDYRATIO_EXTRA_D
