   ...
   !      ******************************************************************
   !
   SUBROUTINE CHECKINPUTPARAM_MOD_EXTRA_D()
   USE COUPLERPARAM
   USE COMMUNICATION
   USE ALLINPUTPARAM
   USE FLOWVARREFSTATE
   ...
   arg1d = veldirfreestreamd(1)*veldirfreestream(1) + veldirfreestream(&
   &      1)*veldirfreestreamd(1) + veldirfreestreamd(2)*veldirfreestream(2)&
   &      + veldirfreestream(2)*veldirfreestreamd(2) + veldirfreestreamd(3)*&
   &      veldirfreestream(3) + veldirfreestream(3)*veldirfreestreamd(3)
   arg1 = veldirfreestream(1)*veldirfreestream(1) + veldirfreestream(2)&
   &      *veldirfreestream(2) + veldirfreestream(3)*veldirfreestream(3)
   IF (arg1 .EQ. 0.0) THEN
   veclengthd = 0.0
   ELSE
   veclengthd = arg1d/(2.0*SQRT(arg1))
   END IF
   veclength = SQRT(arg1)
   IF (veclength .LT. eps) THEN
   IF (myid .EQ. 0) CALL TERMINATE('checkInputParam', &
   &                               'Free stream velocity direction wrongly '&
   &                                   , 'specified')
   CALL MPI_BARRIER(sumb_comm_world, ierr)
   END IF
   veclengthd = -(one*veclengthd/veclength**2)
   veclength = one/veclength
   veldirfreestreamd(1) = veldirfreestreamd(1)*veclength + &
   &      veldirfreestream(1)*veclengthd
   veldirfreestream(1) = veldirfreestream(1)*veclength
   veldirfreestreamd(2) = veldirfreestreamd(2)*veclength + &
   &      veldirfreestream(2)*veclengthd
   veldirfreestream(2) = veldirfreestream(2)*veclength
   veldirfreestreamd(3) = veldirfreestreamd(3)*veclength + &
   &      veldirfreestream(3)*veclengthd
   ...
   veldirfreestreamd(1) = 0.0
   veldirfreestream(1) = one
   veldirfreestreamd(2) = 0.0
   veldirfreestream(2) = zero
   veldirfreestreamd(3) = 0.0
   ...
   arg1 = liftdirection(1)*liftdirection(1) + liftdirection(2)*&
   &      liftdirection(2) + liftdirection(3)*liftdirection(3)
   veclength = SQRT(arg1)
   IF (veclength .LT. eps) THEN
   IF (myid .EQ. 0) CALL TERMINATE('checkInputParam', &
   ...
   dot = liftdirection(1)*dragdirection(1) + liftdirection(2)*&
   &      dragdirection(2) + liftdirection(3)*dragdirection(3)
   IF (dot .GE. 0.) THEN
   abs1 = dot
   ELSE
   abs1 = -dot
   END IF
   IF (abs1 .GT. 1.e-3_realType) THEN
   IF (myid .EQ. 0) CALL TERMINATE('checkInputParam', &
   ...
   IF (liftdirection(3) .LT. zero) THEN
   ...
   arg1 = veldirini(1)*veldirini(1) + veldirini(2)*veldirini(2) + &
