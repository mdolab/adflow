  ...
  ELSE
     ...
     CASE (nsequations, ransequations) 
        ...
        DO k=0,kb
           DO j=2,jl
              DO i=2,il
                 IF (w(i, j, k, irho) .GT. 0.0 .OR. (w(i, j, k, irho) .LT. &
                      &                0.0 .AND. gamma(i, j, k) .EQ. INT(gamma(i, j, k)))) THEN
                    pwr1d = gamma(i, j, k)*w(i, j, k, irho)**(gamma(i, j, k)-1&
                         &                )*wd(i, j, k, irho)
                 ELSE IF (w(i, j, k, irho) .EQ. 0.0 .AND. gamma(i, j, k) .EQ.&
                      &                1.0) THEN
                    pwr1d = wd(i, j, k, irho)
                 ELSE
                    pwr1d = 0.0
        ...
        DO k=2,kl
           DO j=2,jl
              IF (w(0, j, k, irho) .GT. 0.0 .OR. (w(0, j, k, irho) .LT. 0.0 &
                   &              .AND. gamma(0, j, k) .EQ. INT(gamma(0, j, k)))) THEN
                 pwr1d = gamma(0, j, k)*w(0, j, k, irho)**(gamma(0, j, k)-1)*&
                      &              wd(0, j, k, irho)
              ELSE IF (w(0, j, k, irho) .EQ. 0.0 .AND. gamma(0, j, k) .EQ. &
                   &              1.0) THEN
                 pwr1d = wd(0, j, k, irho)
              ELSE
                 pwr1d = 0.0
              ...
              IF (w(1, j, k, irho) .GT. 0.0 .OR. (w(1, j, k, irho) .LT. 0.0 &
                   &              .AND. gamma(1, j, k) .EQ. INT(gamma(1, j, k)))) THEN
                 pwr1d = gamma(1, j, k)*w(1, j, k, irho)**(gamma(1, j, k)-1)*&
                      &              wd(1, j, k, irho)
              ELSE IF (w(1, j, k, irho) .EQ. 0.0 .AND. gamma(1, j, k) .EQ. &
                   &              1.0) THEN
                 pwr1d = wd(1, j, k, irho)
              ELSE
                 pwr1d = 0.0
              ...
              IF (w(ie, j, k, irho) .GT. 0.0 .OR. (w(ie, j, k, irho) .LT. &
                   &              0.0 .AND. gamma(ie, j, k) .EQ. INT(gamma(ie, j, k)))) THEN
                 pwr1d = gamma(ie, j, k)*w(ie, j, k, irho)**(gamma(ie, j, k)-&
                      &              1)*wd(ie, j, k, irho)
              ELSE IF (w(ie, j, k, irho) .EQ. 0.0 .AND. gamma(ie, j, k) .EQ.&
                   &              1.0) THEN
              ...
              IF (w(ib, j, k, irho) .GT. 0.0 .OR. (w(ib, j, k, irho) .LT. &
                   &              0.0 .AND. gamma(ib, j, k) .EQ. INT(gamma(ib, j, k)))) THEN
                 pwr1d = gamma(ib, j, k)*w(ib, j, k, irho)**(gamma(ib, j, k)-&
                      &              1)*wd(ib, j, k, irho)
              ELSE IF (w(ib, j, k, irho) .EQ. 0.0 .AND. gamma(ib, j, k) .EQ.&
                   &              1.0) THEN
        ...
        DO k=2,kl
           DO i=2,il
              IF (w(i, 0, k, irho) .GT. 0.0 .OR. (w(i, 0, k, irho) .LT. 0.0 &
                   &              .AND. gamma(i, 0, k) .EQ. INT(gamma(i, 0, k)))) THEN
                 pwr1d = gamma(i, 0, k)*w(i, 0, k, irho)**(gamma(i, 0, k)-1)*&
                      &              wd(i, 0, k, irho)
              ELSE IF (w(i, 0, k, irho) .EQ. 0.0 .AND. gamma(i, 0, k) .EQ. &
                   &              1.0) THEN
                 pwr1d = wd(i, 0, k, irho)
              ELSE
                 pwr1d = 0.0
              ...
              IF (w(i, 1, k, irho) .GT. 0.0 .OR. (w(i, 1, k, irho) .LT. 0.0 &
                   &              .AND. gamma(i, 1, k) .EQ. INT(gamma(i, 1, k)))) THEN
                 pwr1d = gamma(i, 1, k)*w(i, 1, k, irho)**(gamma(i, 1, k)-1)*&
                      &              wd(i, 1, k, irho)
              ELSE IF (w(i, 1, k, irho) .EQ. 0.0 .AND. gamma(i, 1, k) .EQ. &
                   &              1.0) THEN
                 pwr1d = wd(i, 1, k, irho)
              ELSE
                 pwr1d = 0.0
              ...
              IF (w(i, je, k, irho) .GT. 0.0 .OR. (w(i, je, k, irho) .LT. &
                   &              0.0 .AND. gamma(i, je, k) .EQ. INT(gamma(i, je, k)))) THEN
                 pwr1d = gamma(i, je, k)*w(i, je, k, irho)**(gamma(i, je, k)-&
                      &              1)*wd(i, je, k, irho)
              ELSE IF (w(i, je, k, irho) .EQ. 0.0 .AND. gamma(i, je, k) .EQ.&
                   &              1.0) THEN
              ...
              IF (w(i, jb, k, irho) .GT. 0.0 .OR. (w(i, jb, k, irho) .LT. &
                   &              0.0 .AND. gamma(i, jb, k) .EQ. INT(gamma(i, jb, k)))) THEN
                 pwr1d = gamma(i, jb, k)*w(i, jb, k, irho)**(gamma(i, jb, k)-&
                      &              1)*wd(i, jb, k, irho)
              ELSE IF (w(i, jb, k, irho) .EQ. 0.0 .AND. gamma(i, jb, k) .EQ.&
                   &              1.0) THEN
