   ...
   !
   !  Differentiation of bcturbwall in forward (tangent) mode:
   ...
   ii = 2_intType
   END IF
   nud = -(rlv2(i, j)*ww2d(i, j, irho)/ww2(i, j, irho)**2)
   ...
   abs1d = tmped*ww2(i, j, itu1) + tmpe*ww2d(i, j, itu1)
   abs1 = tmpe*ww2(i, j, itu1)
   ELSE
   abs1d = -(tmped*ww2(i, j, itu1)+tmpe*ww2d(i, j, itu1))
   abs1 = -(tmpe*ww2(i, j, itu1))
   END IF
   tmpfd = -((20.0_realType*2*nu*tmpd**2*nud*abs1-20.0_realType*nu&
   &          **2*tmpd**2*abs1d)/abs1**2)
