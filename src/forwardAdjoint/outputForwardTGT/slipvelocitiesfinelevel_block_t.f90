   ...
   !
   !  Differentiation of slipvelocitiesfinelevel_block in forward (tangent) mode (with options debugTangent i4 dr8 r8):
   ...
   !      ******************************************************************
   !
   SUBROUTINE SLIPVELOCITIESFINELEVEL_BLOCK_T(useoldcoor, t, sps)
   USE ITERATION
   USE INPUTMOTION
   USE FLOWVARREFSTATE
   USE MONITOR
   ...
   &                              , veldirfreestreamd, 3)
   CALL DEBUG_TGT_REAL8('machgrid', machgrid, machgridd)
   CALL DEBUG_TGT_REAL8('velxgrid0', velxgrid0, velxgrid0d)
   CALL DEBUG_TGT_REAL8('velzgrid0', velzgrid0, velzgrid0d)
   CALL DEBUG_TGT_REAL8('ainf', ainf, ainfd)
   CALL DEBUG_TGT_REAL8ARRAY('derivrotationmatrix', &
   &                              derivrotationmatrix, derivrotationmatrixd&
   &                              , 3**2)
   CALL DEBUG_TGT_REAL8('velygrid0', velygrid0, velygrid0d)
   ...
   CALL GETDIRVECTOR_T(refdirection, alphats, alphatsd, beta, &
   ...
   refdirection(1) = one
   CALL DEBUG_TGT_CALL('GETDIRVECTOR', .TRUE., .FALSE.)
   CALL GETDIRVECTOR_T(refdirection, alpha, alphad, betats, &
   ...
   END IF
   END SUBROUTINE SLIPVELOCITIESFINELEVEL_BLOCK_T
