   ...
   !
   !  Differentiation of metric_block in forward (tangent) mode (with options debugTangent i4 dr8 r8):
   ...
   CALL DEBUG_TGT_CALL('VOLPYM', .TRUE., .FALSE.)
   ! Compute the volumes of the 6 sub pyramids. The
   ! arguments of volpym must be such that for a (regular)
   ! right handed hexahedron all volumes are positive.
   CALL VOLPYM_T(x(i, j, k, 1), xd(i, j, k, 1), x(i, j, k, 2), xd(i&
   ...
   CALL DEBUG_TGT_EXIT()
   CALL DEBUG_TGT_CALL('VOLPYM', .TRUE., .FALSE.)
   CALL VOLPYM_T(x(l, j, k, 1), xd(l, j, k, 1), x(l, j, k, 2), xd(l&
   ...
   CALL DEBUG_TGT_EXIT()
   CALL DEBUG_TGT_CALL('VOLPYM', .TRUE., .FALSE.)
   CALL VOLPYM_T(x(i, m, k, 1), xd(i, m, k, 1), x(i, m, k, 2), xd(i&
   ...
   CALL DEBUG_TGT_EXIT()
   CALL DEBUG_TGT_CALL('VOLPYM', .TRUE., .FALSE.)
   CALL VOLPYM_T(x(i, j, n, 1), xd(i, j, n, 1), x(i, j, n, 2), xd(i&
   ...
   END IF
      CONTAINS
   !  Differentiation of volpym in forward (tangent) mode (with options debugTangent i4 dr8 r8):
   !   variations   of useful results: volume
   !   with respect to varying inputs: xp yp zp xa xb xc xd ya yb
   !                yc yd za zb zc zd
   !        ================================================================
   SUBROUTINE VOLPYM_T(xa, xad, ya, yad, za, zad, xb, xbd, yb, ybd, zb, &
   ...
   CALL DEBUG_TGT_REAL8('xa', xa, xad)
   CALL DEBUG_TGT_REAL8('xb', xb, xbd)
   CALL DEBUG_TGT_REAL8('xc', xc, xcd)
   CALL DEBUG_TGT_REAL8('xd', xd, xdd)
   CALL DEBUG_TGT_REAL8('ya', ya, yad)
   CALL DEBUG_TGT_REAL8('yb', yb, ybd)
   CALL DEBUG_TGT_REAL8('yc', yc, ycd)
   CALL DEBUG_TGT_REAL8('yd', yd, ydd)
   CALL DEBUG_TGT_REAL8('za', za, zad)
   CALL DEBUG_TGT_REAL8('zb', zb, zbd)
   CALL DEBUG_TGT_REAL8('zc', zc, zcd)
   CALL DEBUG_TGT_REAL8('zd', zd, zdd)
   CALL DEBUG_TGT_DISPLAY('entry')
   END IF
   !
   !        ****************************************************************
   ...
   IF (.TRUE. .AND. DEBUG_TGT_HERE('exit', .FALSE.)) THEN
   CALL DEBUG_TGT_REAL8('volume', volume, volumed)
   CALL DEBUG_TGT_DISPLAY('exit')
   END IF
   END SUBROUTINE VOLPYM_T
   !        ================================================================
