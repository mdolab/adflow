   ...
   !
   !  Differentiation of invisciddissfluxmatrixcoarse in forward (tangent) mode (with options debugTangent i4 dr8 r8):
   ...
   !      ******************************************************************
   !
   SUBROUTINE INVISCIDDISSFLUXMATRIXCOARSE_T()
   USE CONSTANTS
   USE ITERATION
   USE FLOWVARREFSTATE
   USE BLOCKPOINTERS_D
   ...
   REAL(kind=realtype) :: gammaavgd, gm1d, ovgm1d, gm53d, tmpd, fsd
   ...
   CALL DEBUG_TGT_REAL8('kavg', kavg, kavgd)
   ...
   CALL DEBUG_TGT_DISPLAY('exit')
   END IF
   END SUBROUTINE INVISCIDDISSFLUXMATRIXCOARSE_T
