   ...
   !      ******************************************************************
   !
   SUBROUTINE INVISCIDUPWINDFLUX_T(finegrid)
   USE CONSTANTS
   USE ITERATION
   USE FLOWVARREFSTATE
   USE CGNSGRID
   ...
   CALL DEBUG_TGT_CALL('RIEMANNFLUX', .TRUE., .FALSE.)
   ! Compute the dissipative flux across the interface.
   CALL RIEMANNFLUX_T(left, leftd, right, rightd, flux, fluxd)
   CALL DEBUG_TGT_EXIT()
   ...
   CALL DEBUG_TGT_REAL8ARRAY('right', right, rightd, nw)
   ...
   END IF
   CALL DEBUG_TGT_CALL('LEFTRIGHTSTATE', .TRUE., .FALSE.)
   ! Compute the differences from the first order scheme.
   CALL LEFTRIGHTSTATE_T(du1, du1d, du2, du2d, du3, du3d, &
   ...
   END IF
      CONTAINS
   !  Differentiation of leftrightstate in forward (tangent) mode (with options debugTangent i4 dr8 r8):
   !   variations   of useful results: left right du1 du2 du3
   !   with respect to varying inputs: left right du1 du2 du3
   ! Store the density flux in the mass flow of the
   ! appropriate sliding mesh interface.
   !      ==================================================================
   SUBROUTINE LEFTRIGHTSTATE_T(du1, du1d, du2, du2d, du3, du3d, rotmatrix&
   &    , left, leftd, right, rightd)
   USE DIFFSIZES
   !  Hint: ISIZE1OFdu3 should be the size of dimension 1 of array du3
   !  Hint: ISIZE1OFdu2 should be the size of dimension 1 of array du2
   !  Hint: ISIZE1OFdu1 should be the size of dimension 1 of array du1
   !  Hint: ISIZE1OFright should be the size of dimension 1 of array right
   !  Hint: ISIZE1OFleft should be the size of dimension 1 of array left
   IMPLICIT NONE
   !
   !        Local parameter.
   !
   ...
   REAL(kind=realtype), DIMENSION(3, 3) :: rotd
   ...
   IF (.TRUE. .AND. DEBUG_TGT_HERE('entry', .FALSE.)) THEN
   CALL DEBUG_TGT_REAL8ARRAY('left', left, leftd, ISIZE1OFleft)
   CALL DEBUG_TGT_REAL8ARRAY('right', right, rightd, ISIZE1OFright)
   CALL DEBUG_TGT_REAL8ARRAY('du1', du1, du1d, ISIZE1OFdu1)
   CALL DEBUG_TGT_REAL8ARRAY('du2', du2, du2d, ISIZE1OFdu2)
   CALL DEBUG_TGT_REAL8ARRAY('du3', du3, du3d, ISIZE1OFdu3)
   CALL DEBUG_TGT_DISPLAY('entry')
   END IF
   !
   !        ****************************************************************
   ...
   IF (.TRUE. .AND. DEBUG_TGT_HERE('middle', .FALSE.)) THEN
   CALL DEBUG_TGT_REAL8ARRAY('left', left, leftd, ISIZE1OFleft)
   CALL DEBUG_TGT_REAL8ARRAY('right', right, rightd, &
   &                                ISIZE1OFright)
   CALL DEBUG_TGT_REAL8ARRAY('du1', du1, du1d, ISIZE1OFdu1)
   CALL DEBUG_TGT_REAL8ARRAY('du2', du2, du2d, ISIZE1OFdu2)
   CALL DEBUG_TGT_REAL8ARRAY('du3', du3, du3d, ISIZE1OFdu3)
   ...
   CALL DEBUG_TGT_DISPLAY('exit')
   END IF
   END SUBROUTINE LEFTRIGHTSTATE_T
   ! Store the density flux in the mass flow of the
   ! appropriate sliding mesh interface.
   !      ==================================================================
   ...
   !  Hint: ISIZE1OFright should be the size of dimension 1 of array right
   !  Hint: ISIZE1OFleft should be the size of dimension 1 of array left
   !  Hint: ISIZE1OFflux should be the size of dimension 1 of array flux
   IMPLICIT NONE
   !
   !        Subroutine arguments.
   !
   ...
   CALL ETOTARRAY_T(rhotmp, rhotmpd, utmp, utmpd, vtmp, vtmpd, wtmp&
   ...
   CALL DEBUG_TGT_REAL8('sy', sy, syd)
   CALL DEBUG_TGT_REAL8('sz', sz, szd)
   CALL DEBUG_TGT_REAL8ARRAY('flux', flux, fluxd, ISIZE1OFflux)
   CALL DEBUG_TGT_REAL8('aavg', aavg, aavgd)
   CALL DEBUG_TGT_REAL8('wavg', wavg, wavgd)
   CALL DEBUG_TGT_REAL8('area', area, aread)
   CALL DEBUG_TGT_REAL8('gm1', gm1, gm1d)
   CALL DEBUG_TGT_REAL8('dr', dr, drd)
   CALL DEBUG_TGT_REAL8('gm53', gm53, gm53d)
   CALL DEBUG_TGT_REAL8('vavg', vavg, vavgd)
   CALL DEBUG_TGT_REAL8('abs1', abs1, abs1d)
   CALL DEBUG_TGT_REAL8('uavg', uavg, uavgd)
   CALL DEBUG_TGT_REAL8('dre', dre, dred)
   CALL DEBUG_TGT_REAL8('drk', drk, drkd)
   CALL DEBUG_TGT_REAL8('havg', havg, havgd)
   CALL DEBUG_TGT_REAL8('unavg', unavg, unavgd)
   CALL DEBUG_TGT_REAL8('ovaavg', ovaavg, ovaavgd)
   CALL DEBUG_TGT_REAL8('dru', dru, drud)
   CALL DEBUG_TGT_REAL8('drv', drv, drvd)
   CALL DEBUG_TGT_REAL8('drw', drw, drwd)
   CALL DEBUG_TGT_REAL8('rface', rface, rfaced)
   CALL DEBUG_TGT_REAL8('x2', x2, x2d)
   CALL DEBUG_TGT_REAL8('ova2avg', ova2avg, ova2avgd)
   CALL DEBUG_TGT_REAL8('alphaavg', alphaavg, alphaavgd)
   CALL DEBUG_TGT_DISPLAY('middle')
   END IF
   abs2d = -x2d
   abs2 = -x2
   ...
   END SUBROUTINE INVISCIDUPWINDFLUX_T
