   ...
   !
   !  Differentiation of sasolve in forward (tangent) mode (with options debugTangent i4 dr8 r8):
   ...
   REAL(kind=realtype) :: uud, umd, upd, fd, tu1pd
   ...
   CASE (strain) 
   CALL DEBUG_TGT_CALL('PRODSMAG2', .TRUE., .FALSE.)
   CALL PRODSMAG2_T()
   CALL DEBUG_TGT_EXIT()
   qqd = 0.0_8
   CASE (vorticity) 
   CALL DEBUG_TGT_CALL('PRODWMAG2', .TRUE., .FALSE.)
   CALL PRODWMAG2_T()
   CALL DEBUG_TGT_EXIT()
   qqd = 0.0_8
   CASE (katolaunder) 
   CALL DEBUG_TGT_CALL('PRODKATOLAUNDER', .TRUE., .FALSE.)
   CALL PRODKATOLAUNDER_T()
   CALL DEBUG_TGT_EXIT()
   qqd = 0.0_8
   CASE DEFAULT
   qqd = 0.0_8
   ...
   !
   nn = itu1 - 1
   CALL DEBUG_TGT_CALL('TURBADVECTION', .TRUE., .FALSE.)
   CALL TURBADVECTION_T(1_intType, 1_intType, nn, qq, qqd)
   CALL DEBUG_TGT_EXIT()
   CALL DEBUG_TGT_CALL('UNSTEADYTURBTERM', .TRUE., .FALSE.)
   CALL UNSTEADYTURBTERM_T(1_intType, 1_intType, nn, qq, qqd)
   CALL DEBUG_TGT_EXIT()
   !
   !      ******************************************************************
   !      *                                                                *
   ...
   &                                ))
   CALL DEBUG_TGT_REAL8('tu1p', tu1p, tu1pd)
   ...
   &            j)/rrlv(i, j)
   CALL DEBUG_TGT_CALL('CURVETUPYP', .TRUE., .FALSE.)
   CALL CURVETUPYP_T(tu1p, tu1pd, yp, ypd, itu1, itu1)
   CALL DEBUG_TGT_EXIT()
   ddvtd(i, j, 1) = ((tu1pd*rrlv(i, j)+tu1p*rrlvd(i, j))*ww(i, j&
   &            , irho)-tu1p*rrlv(i, j)*wwd(i, j, irho))/ww(i, j, irho)**2 -&
   &            wwd(i, j, itu1)
