   ...
   tmpb8 = x0b(i+1, j+1, 3)
   x0b(i+1, j+1, 3) = 0.0_8
   x2b(i+1, j+1, 3) = x2b(i+1, j+1, 3) + tmpb8
   normb(3) = normb(3) + dot*tmpb8
   tmpb9 = x0b(i+1, j+1, 2)
   x0b(i+1, j+1, 2) = 0.0_8
   x2b(i+1, j+1, 2) = x2b(i+1, j+1, 2) + tmpb9
   normb(2) = normb(2) + dot*tmpb9
   tmpb10 = x0b(i+1, j+1, 1)
   dotb = norm(2)*tmpb9 + norm(1)*tmpb10 + norm(3)*tmpb8
   x0b(i+1, j+1, 1) = 0.0_8
   x2b(i+1, j+1, 1) = x2b(i+1, j+1, 1) + tmpb10
   tempb0 = two*dotb
   normb(1) = normb(1) + v1(1)*tempb0 + dot*tmpb10
