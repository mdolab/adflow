   ...
   &                            rotmatrixk, rotmatrixkb, left, leftb, right&
   &                            , rightb)
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   ...
   CALL POPREAL8ARRAY(right, nw)
   CALL LEFTRIGHTSTATE_B(du1, du1b, du2, du2b, du3, du3b, &
   &                            rotmatrixj, rotmatrixjb, left, leftb, right&
   &                            , rightb)
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   ...
   CALL POPREAL8ARRAY(right, nw)
   CALL LEFTRIGHTSTATE_B(du1, du1b, du2, du2b, du3, du3b, &
   &                            rotmatrixi, rotmatrixib, left, leftb, right&
   &                            , rightb)
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   ...
   gammaconstantb = 0.0_8
      CONTAINS
   !  Differentiation of leftrightstate in reverse (adjoint) mode:
   ...
   &    , rotmatrixb, left, leftb, right, rightb)
   IMPLICIT NONE
   !
   !        Local parameter.
   !
   ...
   REAL(kind=realtype), DIMENSION(:, :, :, :, :), POINTER :: rotmatrixb
   !
   !        Local variables.
   !
   ...
   max5b = SIGN(1.d0, max5*du3(l))*temp1b
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   CALL POPREAL8(max5)
   x4b = 0.0_8
   ...
   max4b = SIGN(1.d0, max4*du1(l))*temp0b
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   CALL POPREAL8(max4)
   x3b = 0.0_8
   ...
   max7b = SIGN(1.d0, max7*du3(l))*temp4b
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   CALL POPREAL8(max7)
   x6b = 0.0_8
   ...
   max6b = SIGN(1.d0, max6*du1(l))*temp3b
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   CALL POPREAL8(max6)
   x5b = 0.0_8
   ...
   max3b = SIGN(1.d0, max3*du2(l))*temp2b3
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   CALL POPREAL8(max3)
   x2b = 0.0_8
   ...
   !  Differentiation of riemannflux in reverse (adjoint) mode:
   ...
   fluxb(imz) = 0.0_8
   temp3b6 = -(porflux*fluxb(imy))
   fluxb(imy) = 0.0_8
   temp3b4 = -(porflux*fluxb(imx))
   abv7b = sz*temp3b3 + sx*temp3b4 + sy*temp3b6 + unavg*temp3b2
   fluxb(imx) = 0.0_8
   temp3b5 = -(porflux*fluxb(irho))
   abv6b = wavg*temp3b3 + uavg*temp3b4 + temp3b5 + vavg*temp3b6 + &
   &          havg*temp3b2
   fluxb(irho) = 0.0_8
   ...
   lam3b = drw*temp3b3 + dru*temp3b4 - abv3b + dr*temp3b5 + drv*&
   &          temp3b6 + dre*temp3b2
   abv5b = ovaavg*abv2*abv6b + abv3*abv7b
   unavgb = abv7*temp3b2 - dr*abv5b
   szb = drw*abv5b + abv7*temp3b3
   syb = drv*abv5b + abv7*temp3b6
   sxb = dru*abv5b + abv7*temp3b4
   ova2avgb = abv3*abv4*abv6b
   temp3b7 = gm1*abv4b
   dreb = temp3b7 + lam3*temp3b2
   drwb = sz*abv5b - wavg*temp3b7 + lam3*temp3b3
   wavgb = abv6*temp3b3 - drw*temp3b7
   drvb = sy*abv5b - vavg*temp3b7 + lam3*temp3b6
   vavgb = abv6*temp3b6 - drv*temp3b7
   drub = sx*abv5b - uavg*temp3b7 + lam3*temp3b4
   uavgb = abv6*temp3b4 - dru*temp3b7
   drb = alphaavg*temp3b7 - unavg*abv5b + lam3*temp3b5
   ...
   temp1b1 = 0.0
   ELSE
   temp1b1 = gammaface*x2b/(2.0*SQRT(gammaface*temp1)*left(irho))
   END IF
   temp2 = right(irhoe)/right(irho)
   IF (gammaface*temp2 .EQ. 0.0) THEN
   temp2b = 0.0
   ELSE
   temp2b = -(gammaface*x2b/(2.0*SQRT(gammaface*temp2)*right(irho&
   &            )))
   END IF
   leftb(irhoe) = leftb(irhoe) + temp1b1
   leftb(irho) = leftb(irho) - temp1*temp1b1
   rightb(irhoe) = rightb(irhoe) + temp2b
   rightb(irho) = rightb(irho) - temp2*temp2b
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   x1b = -abs1b
   ELSE
   ...
   z1lb = left(ivz)*tempb3 + left(ivx)*tempb4 + left(ivy)*tempb5 - &
   &          temp*tempb2
   etrb = dreb + temp0b
   rightb(irhoe) = rightb(irhoe) + temp0b
   z1rb = right(ivz)*tempb3 + right(ivx)*tempb4 + right(ivy)*tempb5&
   &          - temp0*temp0b
   leftb(ivz) = leftb(ivz) + z1l*tempb3
   rightb(ivz) = rightb(ivz) + z1r*tempb3
   leftb(ivy) = leftb(ivy) + z1l*tempb5
   rightb(ivy) = rightb(ivy) + z1r*tempb5
   ...
   z1lb = z1lb + tempb
   z1rb = z1rb + tempb
   IF (.NOT.right(irho) .EQ. 0.0) rightb(irho) = rightb(irho) + &
