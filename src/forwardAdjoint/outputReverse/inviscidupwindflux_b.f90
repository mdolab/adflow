   ...
   !   with respect to varying inputs: tref rgas gammaface flux left
   !                right
   !        ================================================================
   ...
   ! 0.0 or 0.5*rFil.
   tempb13 = -(porflux*fluxb(irhoe))
   havgb = abv6*tempb13
   fluxb(irhoe) = 0.0_8
   tempb14 = -(porflux*fluxb(imz))
   fluxb(imz) = 0.0_8
   tempb17 = -(porflux*fluxb(imy))
   fluxb(imy) = 0.0_8
   tempb15 = -(porflux*fluxb(imx))
   abv7b = sz*tempb14 + sx*tempb15 + sy*tempb17 + unavg*tempb13
   fluxb(imx) = 0.0_8
   tempb16 = -(porflux*fluxb(irho))
   abv6b = wavg*tempb14 + uavg*tempb15 + tempb16 + vavg*tempb17 + &
   &         havg*tempb13
   fluxb(irho) = 0.0_8
   ...
   lam3b = drw*tempb14 + dru*tempb15 - abv3b + dr*tempb16 + drv*&
   &         tempb17 + dre*tempb13
   abv5b = ovaavg*abv2*abv6b + abv3*abv7b
   unavgb = abv7*tempb13 - dr*abv5b
   ova2avgb = abv3*abv4*abv6b
   tempb18 = gm1*abv4b
   dreb = tempb18 + lam3*tempb13
   drwb = sz*abv5b - wavg*tempb18 + lam3*tempb14
   wavgb = abv6*tempb14 - drw*tempb18
   drvb = sy*abv5b - vavg*tempb18 + lam3*tempb17
   vavgb = abv6*tempb17 - drv*tempb18
   drub = sx*abv5b - uavg*tempb18 + lam3*tempb15
   uavgb = abv6*tempb15 - dru*tempb18
   drb = alphaavg*tempb18 - unavg*abv5b + lam3*tempb16
   gm1b = (alphaavg*dr-uavg*dru-vavg*drv+dre-wavg*drw)*abv4b
   alphaavgb = dr*tempb18
   gm53b = -(drk*abv4b)
   drkb = -(gm53*abv4b)
   abv1b = abv3b
   ...
   IF (branch .EQ. 0) THEN
   CALL POPREAL8(lam3)
   tempb12 = fourth*lam3b/eta
   etab = lam3b - lam3**2*tempb12/eta
   lam3b = 2*lam3*tempb12
   ELSE
   etab = 0.0_8
   END IF
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   CALL POPREAL8(lam2)
   tempb11 = fourth*lam2b/eta
   etab = etab + lam2b - lam2**2*tempb11/eta
   lam2b = 2*lam2*tempb11
   END IF
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   CALL POPREAL8(lam1)
   tempb10 = fourth*lam1b/eta
   etab = etab + lam1b - lam1**2*tempb10/eta
   lam1b = 2*lam1*tempb10
   END IF
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   unavgb = unavgb + lam3b
   ...
   tempb8 = 0.0
   ELSE
   tempb8 = x2b/(2.0*SQRT(temp1)*left(irho))
   END IF
   temp2 = gammaface*right(irhoe)/right(irho)
   IF (temp2 .EQ. 0.0_8) THEN
   tempb9 = 0.0
   ELSE
   tempb9 = -(x2b/(2.0*SQRT(temp2)*right(irho)))
   END IF
   gammafaceb = right(irhoe)*tempb9 + left(irhoe)*tempb8
   leftb(irhoe) = leftb(irhoe) + gammaface*tempb8
   leftb(irho) = leftb(irho) - temp1*tempb8
   rightb(irhoe) = rightb(irhoe) + gammaface*tempb9
   rightb(irho) = rightb(irho) - temp2*tempb9
   CALL POPCONTROL1B(branch)
   IF (branch .EQ. 0) THEN
   x1b = -abs1b
   ELSE
   ...
   alphaavgb = alphaavgb + gm1*a2avgb
   END IF
   tempb7 = half*alphaavgb
   uavgb = uavgb + 2*uavg*tempb7
   vavgb = vavgb + 2*vavg*tempb7
   wavgb = wavgb + 2*wavg*tempb7
   tmp = one/(z1l+z1r)
   tempb5 = tmp*uavgb
   tempb6 = tmp*vavgb
   tempb4 = tmp*wavgb
   ...
   &         temp+temp0)*havgb
   etlb = tempb2 - dreb
   leftb(irhoe) = leftb(irhoe) + tempb2
   z1lb = left(ivz)*tempb4 + left(ivx)*tempb5 + left(ivy)*tempb6 - &
   &         temp*tempb2
   etrb = dreb + tempb3
   rightb(irhoe) = rightb(irhoe) + tempb3
   z1rb = right(ivz)*tempb4 + right(ivx)*tempb5 + right(ivy)*tempb6&
   &         - temp0*tempb3
   leftb(ivz) = leftb(ivz) + z1l*tempb4
   rightb(ivz) = rightb(ivz) + z1r*tempb4
   leftb(ivy) = leftb(ivy) + z1l*tempb6
   rightb(ivy) = rightb(ivy) + z1r*tempb6
   leftb(ivx) = leftb(ivx) + z1l*tempb5
   rightb(ivx) = rightb(ivx) + z1r*tempb5
