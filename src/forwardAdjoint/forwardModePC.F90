...
#include "include/finclude/petsc.h"
  ...
  print *,'starting dw actually:'
  print *,dw(2,2,2,:)
  print *,'with vol:'
  print *,dw(2,2,2,:)/vol(2,2,2)
  ...
  domainLoopAD: do nn=1,nDom
     ...
     ! Setup the coloring stencil for this block depending on if its
     ! drdw or a PC

     if (usePC) then
        !call setup_7point_stencil(nn,nColor)
        !call setup_27point_stencil(nn,nColor)
        !call setup_125point_stencil(nn,nColor)
        call setup_BF_stencil(nn,nColor)
     else
        if( .not. viscous ) then
           ! Euler Stencils
           !call setup_13point_stencil(nn,nColor)
           !call setup_125point_stencil(nn,nColor)
           !call setup_BF_stencil(nn,nColor)
           call setup_BF_stencil(nn,nColor)
        else ! Viscous/RANS
           !call setup_25point_stencil(nn,nColor)
           call setup_125point_stencil(nn,nColor)
           print *,'Not done yet'
           stop
        end if
