...
!                *sk *radi *radj *radk *cdisrk vis4 vis2 vis2coarse
!                sigma
!
!      ******************************************************************
!      *                                                                *
...
!      ******************************************************************
!
SUBROUTINE RESIDUAL_BLOCK_SPATIAL_D()
...
! is the finest grid level of the current mg cycle.
  ...
  IF (finegrid .EQV. .false.) THEN
    ...
    STOP
  ELSE
    CALL INVISCIDCENTRALFLUX_SPATIAL_D()
...
! Standard scalar dissipation scheme.
      IF (finegrid) THEN
        CALL INVISCIDDISSFLUXSCALAR_SPATIAL_D()
      ELSE
        CALL INVISCIDDISSFLUXSCALARCOARSE_SPATIAL_D()
      END IF
    CASE (dissmatrix) 
!===========================================================
! Matrix dissipation scheme.
      IF (finegrid) THEN
        CALL INVISCIDDISSFLUXMATRIX_SPATIAL_D()
      ELSE
        CALL INVISCIDDISSFLUXMATRIXCOARSE_SPATIAL_D()
      END IF
    CASE (disscusp) 
!===========================================================
...
! Dissipation via an upwind scheme.
      CALL INVISCIDUPWINDFLUX_SPATIAL_D(finegrid)
    CASE DEFAULT
      fwd = 0.0
    END SELECT
! Compute the viscous flux in case of a viscous computation.
    IF (viscous) CALL VISCOUSFLUX_SPATIAL_D()
...
END SUBROUTINE RESIDUAL_BLOCK_SPATIAL_D
