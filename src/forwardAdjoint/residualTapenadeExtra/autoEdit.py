...
DIR_ORI = os.getcwd()
DIR_MOD = DIR_ORI + '/../residualOutputExtra'

# Specifiy the list of LINE ID's to find, what to replace and with what

# First set: Find line with 'USE' and replace "EXTRA_D" with ''
LINE_ID = ['USE','USE','CALL','CALL']
STR_OLD = ['BLOCKPOINTERS_EXTRA_D','_EXTRA_D' ,'INSTANCE_EXTRA_D','SETPOINTERS_EXTRA_D']
STR_NEW = [ 'BLOCKPOINTERS_D','' ,'INSTANCE_D' ,'SETPOINTERS']

STR_REPLACE_ALL = {'_CD':''}
# Also, entirely ignore lines with these strings:
LINE_IGNORE = ['USE BLOCKPOINTERS_D']

FILE_IGNORE = ['blockpointers_extra_d.f90']
# Some feedback
