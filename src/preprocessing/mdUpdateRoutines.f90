...
!
     ...
     subroutine setUndisplacedNormals()

       ! This is a bit of a hack routine that uses si0, sj0 and sk0 to
       ! compute the forces on the UNDISPLACED (aerostructrually) grid
       ! to eliminate the dependance of F on the structural displacements
...
!
       ...
       spectral: do sps=1,nTimeIntervalsSpectral
          domains: do nn=1,nDom
             ...
             il = flowDoms(nn,1_intType,sps)%il
             jl = flowDoms(nn,1_intType,sps)%jl
             kl = flowDoms(nn,1_intType,sps)%kl
             
             ie = flowDoms(nn,1_intType,sps)%ie
             je = flowDoms(nn,1_intType,sps)%je
             ke = flowDoms(nn,1_intType,sps)%ke
             
             ib = flowDoms(nn,1_intType,sps)%ib
             jb = flowDoms(nn,1_intType,sps)%jb
             kb = flowDoms(nn,1_intType,sps)%kb

             if (.not. associated(flowDoms(nn,1_intType,sps)%si0)) then
                allocate(flowDoms(nn,1_intType,sps)%si0(0:ie,1:je,1:ke,3), &
                         flowDoms(nn,1_intType,sps)%sj0(1:ie,0:je,1:ke,3), &
                         flowDoms(nn,1_intType,sps)%sk0(1:ie,1:je,0:ke,3))    
             end if

             flowDoms(nn,1_intType,sps)%si0 = flowDoms(nn,1_intType,sps)%si
             flowDoms(nn,1_intType,sps)%sj0 = flowDoms(nn,1_intType,sps)%sj
             flowDoms(nn,1_intType,sps)%sk0 = flowDoms(nn,1_intType,sps)%sk
             

          end do domains

       end do spectral
     end subroutine setUndisplacedNormals
