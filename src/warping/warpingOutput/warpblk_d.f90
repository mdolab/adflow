...
!
!  Differentiation of warpblk in forward (tangent) mode:
!   variations  of output variables: xyz
!   with respect to input variables: dfacei dfacej dfacek xyz
SUBROUTINE WARPBLK_D(ifaceptb, iedgeptb, ncall, ig, igo, il, jl, kl, &
&  xyz0, s0, dfacei, dfaceid, dfacej, dfacejd, dfacek, dfacekd, xyz, xyzd&
&)
  USE PRECISION
  IMPLICIT NONE
!     ******************************************************************
...
! S  For face perturbations; e.g.,
  REAL(kind=realtype) :: dfacei(3, jl, kl, 2, 4)
  REAL(kind=realtype) :: dfaceid(3, jl, kl, 2, 4)
!    DFACEI(1:3,JL,KL,1,M) =
  REAL(kind=realtype) :: dfacej(3, il, kl, 2, 4)
  REAL(kind=realtype) :: dfacejd(3, il, kl, 2, 4)
!    dX, dY, dZ on the I=1 face for
  REAL(kind=realtype) :: dfacek(3, il, jl, 2, 4)
  REAL(kind=realtype) :: dfacekd(3, il, jl, 2, 4)
...
&  delikd, deljid, deljkd, delkid, delkjd
...
!     Parameterize the block volume on the first call:
  IF (ncall .LE. -3 .AND. ig .NE. igo) CALL PARAM3DM(il, jl, kl, xyz0&
&                                                  , s0)
...
&                dfacei(:, :, :, faceindex, 1), dfaceid(:, :, :, &
&                faceindex, 1), dfacej(:, :, :, faceindex, 1), dfacejd(:&
&                , :, :, faceindex, 1), dfacek(:, :, :, faceindex, 1), &
&                dfacekd(:, :, :, faceindex, 1), xyz, xyzd)
    END IF
  END DO
...
!     DFACEI(1,1,1,1,1), DFACEJ, DFACEK, XYZ)
  CALL DELQ3DM_D(il, jl, kl, 1, 1, 1, jl, 1, kl, xyz0, s0, dfacei(:, :, &
&           :, 1, 1), dfaceid(:, :, :, 1, 1), dfacej(:, :, :, 1, 1), &
&           dfacejd(:, :, :, 1, 1), dfacek(:, :, :, 1, 1), dfacekd(:, :, &
&           :, 1, 1), xyz, xyzd)
...
&           , :, 2, 1), dfaceid(:, :, :, 2, 1), dfacej(:, :, :, 2, 1), &
&           dfacejd(:, :, :, 2, 1), dfacek(:, :, :, 2, 1), dfacekd(:, :, &
&           :, 2, 1), xyz, xyzd)
...
!        K = 1 and KL edge perturbations (J direction edges):
    DO j=2,jl-1
      dfaceid(1, j, 1, l, 2) = xyzd(1, i, j, 1) - dfaceid(1, j, 1, l, 1)
      dfacei(1, j, 1, l, 2) = xyz(1, i, j, 1) - xyz0(1, i, j, 1) - &
&        dfacei(1, j, 1, l, 1)
      dfaceid(2, j, 1, l, 2) = xyzd(2, i, j, 1) - dfaceid(2, j, 1, l, 1)
      dfacei(2, j, 1, l, 2) = xyz(2, i, j, 1) - xyz0(2, i, j, 1) - &
&        dfacei(2, j, 1, l, 1)
      dfaceid(3, j, 1, l, 2) = xyzd(3, i, j, 1) - dfaceid(3, j, 1, l, 1)
      dfacei(3, j, 1, l, 2) = xyz(3, i, j, 1) - xyz0(3, i, j, 1) - &
&        dfacei(3, j, 1, l, 1)
      dfaceid(1, j, kl, l, 2) = xyzd(1, i, j, kl) - dfaceid(1, j, kl, l&
&        , 1)
      dfacei(1, j, kl, l, 2) = xyz(1, i, j, kl) - xyz0(1, i, j, kl) - &
&        dfacei(1, j, kl, l, 1)
      dfaceid(2, j, kl, l, 2) = xyzd(2, i, j, kl) - dfaceid(2, j, kl, l&
&        , 1)
      dfacei(2, j, kl, l, 2) = xyz(2, i, j, kl) - xyz0(2, i, j, kl) - &
&        dfacei(2, j, kl, l, 1)
      dfaceid(3, j, kl, l, 2) = xyzd(3, i, j, kl) - dfaceid(3, j, kl, l&
&        , 1)
...
!        J = 1 and JL edge perturbations (K direction edges):
    DO k=2,kl-1
      dfaceid(1, 1, k, l, 3) = xyzd(1, i, 1, k) - dfaceid(1, 1, k, l, 1)
      dfacei(1, 1, k, l, 3) = xyz(1, i, 1, k) - xyz0(1, i, 1, k) - &
&        dfacei(1, 1, k, l, 1)
      dfaceid(2, 1, k, l, 3) = xyzd(2, i, 1, k) - dfaceid(2, 1, k, l, 1)
      dfacei(2, 1, k, l, 3) = xyz(2, i, 1, k) - xyz0(2, i, 1, k) - &
&        dfacei(2, 1, k, l, 1)
      dfaceid(3, 1, k, l, 3) = xyzd(3, i, 1, k) - dfaceid(3, 1, k, l, 1)
      dfacei(3, 1, k, l, 3) = xyz(3, i, 1, k) - xyz0(3, i, 1, k) - &
&        dfacei(3, 1, k, l, 1)
      dfaceid(1, jl, k, l, 3) = xyzd(1, i, jl, k) - dfaceid(1, jl, k, l&
&        , 1)
      dfacei(1, jl, k, l, 3) = xyz(1, i, jl, k) - xyz0(1, i, jl, k) - &
&        dfacei(1, jl, k, l, 1)
      dfaceid(2, jl, k, l, 3) = xyzd(2, i, jl, k) - dfaceid(2, jl, k, l&
&        , 1)
      dfacei(2, jl, k, l, 3) = xyz(2, i, jl, k) - xyz0(2, i, jl, k) - &
&        dfacei(2, jl, k, l, 1)
      dfaceid(3, jl, k, l, 3) = xyzd(3, i, jl, k) - dfaceid(3, jl, k, l&
&        , 1)
...
!        K = 1 and KL edge perturbations (I direction):
    DO i=2,il-1
      dfacejd(1, i, 1, l, 2) = xyzd(1, i, j, 1) - dfacejd(1, i, 1, l, 1)
      dfacej(1, i, 1, l, 2) = xyz(1, i, j, 1) - xyz0(1, i, j, 1) - &
&        dfacej(1, i, 1, l, 1)
      dfacejd(2, i, 1, l, 2) = xyzd(2, i, j, 1) - dfacejd(2, i, 1, l, 1)
      dfacej(2, i, 1, l, 2) = xyz(2, i, j, 1) - xyz0(2, i, j, 1) - &
&        dfacej(2, i, 1, l, 1)
      dfacejd(3, i, 1, l, 2) = xyzd(3, i, j, 1) - dfacejd(3, i, 1, l, 1)
      dfacej(3, i, 1, l, 2) = xyz(3, i, j, 1) - xyz0(3, i, j, 1) - &
&        dfacej(3, i, 1, l, 1)
      dfacejd(1, i, kl, l, 2) = xyzd(1, i, j, kl) - dfacejd(1, i, kl, l&
&        , 1)
      dfacej(1, i, kl, l, 2) = xyz(1, i, j, kl) - xyz0(1, i, j, kl) - &
&        dfacej(1, i, kl, l, 1)
      dfacejd(2, i, kl, l, 2) = xyzd(2, i, j, kl) - dfacejd(2, i, kl, l&
&        , 1)
      dfacej(2, i, kl, l, 2) = xyz(2, i, j, kl) - xyz0(2, i, j, kl) - &
&        dfacej(2, i, kl, l, 1)
      dfacejd(3, i, kl, l, 2) = xyzd(3, i, j, kl) - dfacejd(3, i, kl, l&
&        , 1)
...
!        I = 1 and IL edge perturbations (K direction):
    DO k=2,kl-1
      dfacejd(1, 1, k, l, 3) = xyzd(1, 1, j, k) - dfacejd(1, 1, k, l, 1)
      dfacej(1, 1, k, l, 3) = xyz(1, 1, j, k) - xyz0(1, 1, j, k) - &
&        dfacej(1, 1, k, l, 1)
      dfacejd(2, 1, k, l, 3) = xyzd(2, 1, j, k) - dfacejd(2, 1, k, l, 1)
      dfacej(2, 1, k, l, 3) = xyz(2, 1, j, k) - xyz0(2, 1, j, k) - &
&        dfacej(2, 1, k, l, 1)
      dfacejd(3, 1, k, l, 3) = xyzd(3, 1, j, k) - dfacejd(3, 1, k, l, 1)
      dfacej(3, 1, k, l, 3) = xyz(3, 1, j, k) - xyz0(3, 1, j, k) - &
&        dfacej(3, 1, k, l, 1)
      dfacejd(1, il, k, l, 3) = xyzd(1, il, j, k) - dfacejd(1, il, k, l&
&        , 1)
      dfacej(1, il, k, l, 3) = xyz(1, il, j, k) - xyz0(1, il, j, k) - &
&        dfacej(1, il, k, l, 1)
      dfacejd(2, il, k, l, 3) = xyzd(2, il, j, k) - dfacejd(2, il, k, l&
&        , 1)
      dfacej(2, il, k, l, 3) = xyz(2, il, j, k) - xyz0(2, il, j, k) - &
&        dfacej(2, il, k, l, 1)
      dfacejd(3, il, k, l, 3) = xyzd(3, il, j, k) - dfacejd(3, il, k, l&
&        , 1)
...
!        J = 1 and JL edge perturbations (I direction):
    DO i=2,il-1
      dfacekd(1, i, 1, l, 2) = xyzd(1, i, 1, k) - dfacekd(1, i, 1, l, 1)
      dfacek(1, i, 1, l, 2) = xyz(1, i, 1, k) - xyz0(1, i, 1, k) - &
&        dfacek(1, i, 1, l, 1)
      dfacekd(2, i, 1, l, 2) = xyzd(2, i, 1, k) - dfacekd(2, i, 1, l, 1)
      dfacek(2, i, 1, l, 2) = xyz(2, i, 1, k) - xyz0(2, i, 1, k) - &
&        dfacek(2, i, 1, l, 1)
      dfacekd(3, i, 1, l, 2) = xyzd(3, i, 1, k) - dfacekd(3, i, 1, l, 1)
      dfacek(3, i, 1, l, 2) = xyz(3, i, 1, k) - xyz0(3, i, 1, k) - &
&        dfacek(3, i, 1, l, 1)
      dfacekd(1, i, jl, l, 2) = xyzd(1, i, jl, k) - dfacekd(1, i, jl, l&
&        , 1)
      dfacek(1, i, jl, l, 2) = xyz(1, i, jl, k) - xyz0(1, i, jl, k) - &
&        dfacek(1, i, jl, l, 1)
      dfacekd(2, i, jl, l, 2) = xyzd(2, i, jl, k) - dfacekd(2, i, jl, l&
&        , 1)
      dfacek(2, i, jl, l, 2) = xyz(2, i, jl, k) - xyz0(2, i, jl, k) - &
&        dfacek(2, i, jl, l, 1)
      dfacekd(3, i, jl, l, 2) = xyzd(3, i, jl, k) - dfacekd(3, i, jl, l&
&        , 1)
...
!        I = 1 and IL edge perturbations (J direction):
    DO j=2,jl-1
      dfacekd(1, 1, j, l, 3) = xyzd(1, 1, j, k) - dfacekd(1, 1, j, l, 1)
      dfacek(1, 1, j, l, 3) = xyz(1, 1, j, k) - xyz0(1, 1, j, k) - &
&        dfacek(1, 1, j, l, 1)
      dfacekd(2, 1, j, l, 3) = xyzd(2, 1, j, k) - dfacekd(2, 1, j, l, 1)
      dfacek(2, 1, j, l, 3) = xyz(2, 1, j, k) - xyz0(2, 1, j, k) - &
&        dfacek(2, 1, j, l, 1)
      dfacekd(3, 1, j, l, 3) = xyzd(3, 1, j, k) - dfacekd(3, 1, j, l, 1)
      dfacek(3, 1, j, l, 3) = xyz(3, 1, j, k) - xyz0(3, 1, j, k) - &
&        dfacek(3, 1, j, l, 1)
      dfacekd(1, il, j, l, 3) = xyzd(1, il, j, k) - dfacekd(1, il, j, l&
&        , 1)
      dfacek(1, il, j, l, 3) = xyz(1, il, j, k) - xyz0(1, il, j, k) - &
&        dfacek(1, il, j, l, 1)
      dfacekd(2, il, j, l, 3) = xyzd(2, il, j, k) - dfacekd(2, il, j, l&
&        , 1)
      dfacek(2, il, j, l, 3) = xyz(2, il, j, k) - xyz0(2, il, j, k) - &
&        dfacek(2, il, j, l, 1)
      dfacekd(3, il, j, l, 3) = xyzd(3, il, j, k) - dfacekd(3, il, j, l&
&        , 1)
...
!     --------
  DO k=2,kl-1
    DO j=2,jl-1
      dfaceid(1, j, k, 1, 4) = xyzd(1, 1, j, k) - dfaceid(1, j, k, 1, 1)&
&        - dfaceid(1, j, k, 1, 2) - dfaceid(1, j, k, 1, 3)
      dfacei(1, j, k, 1, 4) = xyz(1, 1, j, k) - xyz0(1, 1, j, k) - &
&        dfacei(1, j, k, 1, 1) - dfacei(1, j, k, 1, 2) - dfacei(1, j, k, &
&        1, 3)
      dfaceid(2, j, k, 1, 4) = xyzd(2, 1, j, k) - dfaceid(2, j, k, 1, 1)&
&        - dfaceid(2, j, k, 1, 2) - dfaceid(2, j, k, 1, 3)
      dfacei(2, j, k, 1, 4) = xyz(2, 1, j, k) - xyz0(2, 1, j, k) - &
&        dfacei(2, j, k, 1, 1) - dfacei(2, j, k, 1, 2) - dfacei(2, j, k, &
&        1, 3)
      dfaceid(3, j, k, 1, 4) = xyzd(3, 1, j, k) - dfaceid(3, j, k, 1, 1)&
&        - dfaceid(3, j, k, 1, 2) - dfaceid(3, j, k, 1, 3)
      dfacei(3, j, k, 1, 4) = xyz(3, 1, j, k) - xyz0(3, 1, j, k) - &
&        dfacei(3, j, k, 1, 1) - dfacei(3, j, k, 1, 2) - dfacei(3, j, k, &
&        1, 3)
      dfaceid(1, j, k, 2, 4) = xyzd(1, il, j, k) - dfaceid(1, j, k, 2, 1&
&        ) - dfaceid(1, j, k, 2, 2) - dfaceid(1, j, k, 2, 3)
      dfacei(1, j, k, 2, 4) = xyz(1, il, j, k) - xyz0(1, il, j, k) - &
&        dfacei(1, j, k, 2, 1) - dfacei(1, j, k, 2, 2) - dfacei(1, j, k, &
&        2, 3)
      dfaceid(2, j, k, 2, 4) = xyzd(2, il, j, k) - dfaceid(2, j, k, 2, 1&
&        ) - dfaceid(2, j, k, 2, 2) - dfaceid(2, j, k, 2, 3)
      dfacei(2, j, k, 2, 4) = xyz(2, il, j, k) - xyz0(2, il, j, k) - &
&        dfacei(2, j, k, 2, 1) - dfacei(2, j, k, 2, 2) - dfacei(2, j, k, &
&        2, 3)
      dfaceid(3, j, k, 2, 4) = xyzd(3, il, j, k) - dfaceid(3, j, k, 2, 1&
&        ) - dfaceid(3, j, k, 2, 2) - dfaceid(3, j, k, 2, 3)
      dfacei(3, j, k, 2, 4) = xyz(3, il, j, k) - xyz0(3, il, j, k) - &
&        dfacei(3, j, k, 2, 1) - dfacei(3, j, k, 2, 2) - dfacei(3, j, k, &
&        2, 3)
    END DO
    DO i=2,il-1
      dfacejd(1, i, k, 1, 4) = xyzd(1, i, 1, k) - dfacejd(1, i, k, 1, 1)&
&        - dfacejd(1, i, k, 1, 2) - dfacejd(1, i, k, 1, 3)
      dfacej(1, i, k, 1, 4) = xyz(1, i, 1, k) - xyz0(1, i, 1, k) - &
&        dfacej(1, i, k, 1, 1) - dfacej(1, i, k, 1, 2) - dfacej(1, i, k, &
&        1, 3)
      dfacejd(2, i, k, 1, 4) = xyzd(2, i, 1, k) - dfacejd(2, i, k, 1, 1)&
&        - dfacejd(2, i, k, 1, 2) - dfacejd(2, i, k, 1, 3)
      dfacej(2, i, k, 1, 4) = xyz(2, i, 1, k) - xyz0(2, i, 1, k) - &
&        dfacej(2, i, k, 1, 1) - dfacej(2, i, k, 1, 2) - dfacej(2, i, k, &
&        1, 3)
      dfacejd(3, i, k, 1, 4) = xyzd(3, i, 1, k) - dfacejd(3, i, k, 1, 1)&
&        - dfacejd(3, i, k, 1, 2) - dfacejd(3, i, k, 1, 3)
      dfacej(3, i, k, 1, 4) = xyz(3, i, 1, k) - xyz0(3, i, 1, k) - &
&        dfacej(3, i, k, 1, 1) - dfacej(3, i, k, 1, 2) - dfacej(3, i, k, &
&        1, 3)
      dfacejd(1, i, k, 2, 4) = xyzd(1, i, jl, k) - dfacejd(1, i, k, 2, 1&
&        ) - dfacejd(1, i, k, 2, 2) - dfacejd(1, i, k, 2, 3)
      dfacej(1, i, k, 2, 4) = xyz(1, i, jl, k) - xyz0(1, i, jl, k) - &
&        dfacej(1, i, k, 2, 1) - dfacej(1, i, k, 2, 2) - dfacej(1, i, k, &
&        2, 3)
      dfacejd(2, i, k, 2, 4) = xyzd(2, i, jl, k) - dfacejd(2, i, k, 2, 1&
&        ) - dfacejd(2, i, k, 2, 2) - dfacejd(2, i, k, 2, 3)
      dfacej(2, i, k, 2, 4) = xyz(2, i, jl, k) - xyz0(2, i, jl, k) - &
&        dfacej(2, i, k, 2, 1) - dfacej(2, i, k, 2, 2) - dfacej(2, i, k, &
&        2, 3)
      dfacejd(3, i, k, 2, 4) = xyzd(3, i, jl, k) - dfacejd(3, i, k, 2, 1&
&        ) - dfacejd(3, i, k, 2, 2) - dfacejd(3, i, k, 2, 3)
...
&        2, 3)
  ...
  DO j=2,jl-1
    DO i=2,il-1
      dfacekd(1, i, j, 1, 4) = xyzd(1, i, j, 1) - dfacekd(1, i, j, 1, 1)&
&        - dfacekd(1, i, j, 1, 2) - dfacekd(1, i, j, 1, 3)
      dfacek(1, i, j, 1, 4) = xyz(1, i, j, 1) - xyz0(1, i, j, 1) - &
&        dfacek(1, i, j, 1, 1) - dfacek(1, i, j, 1, 2) - dfacek(1, i, j, &
&        1, 3)
      dfacekd(2, i, j, 1, 4) = xyzd(2, i, j, 1) - dfacekd(2, i, j, 1, 1)&
&        - dfacekd(2, i, j, 1, 2) - dfacekd(2, i, j, 1, 3)
      dfacek(2, i, j, 1, 4) = xyz(2, i, j, 1) - xyz0(2, i, j, 1) - &
&        dfacek(2, i, j, 1, 1) - dfacek(2, i, j, 1, 2) - dfacek(2, i, j, &
&        1, 3)
      dfacekd(3, i, j, 1, 4) = xyzd(3, i, j, 1) - dfacekd(3, i, j, 1, 1)&
&        - dfacekd(3, i, j, 1, 2) - dfacekd(3, i, j, 1, 3)
      dfacek(3, i, j, 1, 4) = xyz(3, i, j, 1) - xyz0(3, i, j, 1) - &
&        dfacek(3, i, j, 1, 1) - dfacek(3, i, j, 1, 2) - dfacek(3, i, j, &
&        1, 3)
      dfacekd(1, i, j, 2, 4) = xyzd(1, i, j, kl) - dfacekd(1, i, j, 2, 1&
&        ) - dfacekd(1, i, j, 2, 2) - dfacekd(1, i, j, 2, 3)
      dfacek(1, i, j, 2, 4) = xyz(1, i, j, kl) - xyz0(1, i, j, kl) - &
&        dfacek(1, i, j, 2, 1) - dfacek(1, i, j, 2, 2) - dfacek(1, i, j, &
&        2, 3)
      dfacekd(2, i, j, 2, 4) = xyzd(2, i, j, kl) - dfacekd(2, i, j, 2, 1&
&        ) - dfacekd(2, i, j, 2, 2) - dfacekd(2, i, j, 2, 3)
      dfacek(2, i, j, 2, 4) = xyz(2, i, j, kl) - xyz0(2, i, j, kl) - &
&        dfacek(2, i, j, 2, 1) - dfacek(2, i, j, 2, 2) - dfacek(2, i, j, &
&        2, 3)
      dfacekd(3, i, j, 2, 4) = xyzd(3, i, j, kl) - dfacekd(3, i, j, 2, 1&
&        ) - dfacekd(3, i, j, 2, 2) - dfacekd(3, i, j, 2, 3)
  ...
  DO k=2,kl-1
    DO j=2,jl-1
      DO i=2,il-1
        ...
        wtk1 = one - wtk2
!              Stage 1:
        delid = wti1*dfaceid(1, j, k, 1, 1) + wti2*dfaceid(1, j, k, 2, 1&
&          )
        deli = wti1*dfacei(1, j, k, 1, 1) + wti2*dfacei(1, j, k, 2, 1)
        deljd = wtj1*dfacejd(1, i, k, 1, 1) + wtj2*dfacejd(1, i, k, 2, 1&
&          )
        delj = wtj1*dfacej(1, i, k, 1, 1) + wtj2*dfacej(1, i, k, 2, 1)
        delkd = wtk1*dfacekd(1, i, j, 1, 1) + wtk2*dfacekd(1, i, j, 2, 1&
&          )
...
!              Stage 2:
        delijd = wtj1*dfacejd(1, i, k, 1, 2) + wtj2*dfacejd(1, i, k, 2, &
&          2)
        delij = wtj1*dfacej(1, i, k, 1, 2) + wtj2*dfacej(1, i, k, 2, 2)
        delikd = wtk1*dfacekd(1, i, j, 1, 2) + wtk2*dfacekd(1, i, j, 2, &
&          2)
...
&          max1-(abs2*delij+abs14*delik)*max1d)/max1**2
        ...
        deljid = wti1*dfaceid(1, j, k, 1, 2) + wti2*dfaceid(1, j, k, 2, &
&          2)
        delji = wti1*dfacei(1, j, k, 1, 2) + wti2*dfacei(1, j, k, 2, 2)
        deljkd = wtk1*dfacekd(1, i, j, 1, 3) + wtk2*dfacekd(1, i, j, 2, &
&          3)
...
&          max2-(abs3*delji+abs15*deljk)*max2d)/max2**2
        ...
        delkid = wti1*dfaceid(1, j, k, 1, 3) + wti2*dfaceid(1, j, k, 2, &
&          3)
        delki = wti1*dfacei(1, j, k, 1, 3) + wti2*dfacei(1, j, k, 2, 3)
        delkjd = wtj1*dfacejd(1, i, k, 1, 3) + wtj2*dfacejd(1, i, k, 2, &
&          3)
...
&          max3-(abs4*delki+abs16*delkj)*max3d)/max3**2
        ...
        del2 = deli + delj + delk
!              Stage 3:
        delid = wti1*dfaceid(1, j, k, 1, 4) + wti2*dfaceid(1, j, k, 2, 4&
&          )
        deli = wti1*dfacei(1, j, k, 1, 4) + wti2*dfacei(1, j, k, 2, 4)
        deljd = wtj1*dfacejd(1, i, k, 1, 4) + wtj2*dfacejd(1, i, k, 2, 4&
&          )
        delj = wtj1*dfacej(1, i, k, 1, 4) + wtj2*dfacej(1, i, k, 2, 4)
        delkd = wtk1*dfacekd(1, i, j, 1, 4) + wtk2*dfacekd(1, i, j, 2, 4&
&          )
...
!              Repeat all stages for Y perturbations:
        delid = wti1*dfaceid(2, j, k, 1, 1) + wti2*dfaceid(2, j, k, 2, 1&
&          )
        deli = wti1*dfacei(2, j, k, 1, 1) + wti2*dfacei(2, j, k, 2, 1)
        deljd = wtj1*dfacejd(2, i, k, 1, 1) + wtj2*dfacejd(2, i, k, 2, 1&
&          )
        delj = wtj1*dfacej(2, i, k, 1, 1) + wtj2*dfacej(2, i, k, 2, 1)
        delkd = wtk1*dfacekd(2, i, j, 1, 1) + wtk2*dfacekd(2, i, j, 2, 1&
&          )
...
&          max11d)/max11**2
        ...
        delijd = wtj1*dfacejd(2, i, k, 1, 2) + wtj2*dfacejd(2, i, k, 2, &
&          2)
        delij = wtj1*dfacej(2, i, k, 1, 2) + wtj2*dfacej(2, i, k, 2, 2)
        delikd = wtk1*dfacekd(2, i, j, 1, 2) + wtk2*dfacekd(2, i, j, 2, &
&          2)
...
&          max4-(abs6*delij+abs18*delik)*max4d)/max4**2
        ...
        deljid = wti1*dfaceid(2, j, k, 1, 2) + wti2*dfaceid(2, j, k, 2, &
&          2)
        delji = wti1*dfacei(2, j, k, 1, 2) + wti2*dfacei(2, j, k, 2, 2)
        deljkd = wtk1*dfacekd(2, i, j, 1, 3) + wtk2*dfacekd(2, i, j, 2, &
&          3)
...
&          max5-(abs7*delji+abs19*deljk)*max5d)/max5**2
        ...
        delkid = wti1*dfaceid(2, j, k, 1, 3) + wti2*dfaceid(2, j, k, 2, &
&          3)
        delki = wti1*dfacei(2, j, k, 1, 3) + wti2*dfacei(2, j, k, 2, 3)
        delkjd = wtj1*dfacejd(2, i, k, 1, 3) + wtj2*dfacejd(2, i, k, 2, &
&          3)
...
&          max6-(abs8*delki+abs20*delkj)*max6d)/max6**2
        ...
        del2d = delid + deljd + delkd
        del2 = deli + delj + delk
        delid = wti1*dfaceid(2, j, k, 1, 4) + wti2*dfaceid(2, j, k, 2, 4&
&          )
        deli = wti1*dfacei(2, j, k, 1, 4) + wti2*dfacei(2, j, k, 2, 4)
        deljd = wtj1*dfacejd(2, i, k, 1, 4) + wtj2*dfacejd(2, i, k, 2, 4&
&          )
        delj = wtj1*dfacej(2, i, k, 1, 4) + wtj2*dfacej(2, i, k, 2, 4)
        delkd = wtk1*dfacekd(2, i, j, 1, 4) + wtk2*dfacekd(2, i, j, 2, 4&
&          )
...
!              ... and for Z perturbations:
        delid = wti1*dfaceid(3, j, k, 1, 1) + wti2*dfaceid(3, j, k, 2, 1&
&          )
        deli = wti1*dfacei(3, j, k, 1, 1) + wti2*dfacei(3, j, k, 2, 1)
        deljd = wtj1*dfacejd(3, i, k, 1, 1) + wtj2*dfacejd(3, i, k, 2, 1&
&          )
        delj = wtj1*dfacej(3, i, k, 1, 1) + wtj2*dfacej(3, i, k, 2, 1)
        delkd = wtk1*dfacekd(3, i, j, 1, 1) + wtk2*dfacekd(3, i, j, 2, 1&
&          )
...
&          max12d)/max12**2
        ...
        delijd = wtj1*dfacejd(3, i, k, 1, 2) + wtj2*dfacejd(3, i, k, 2, &
&          2)
        delij = wtj1*dfacej(3, i, k, 1, 2) + wtj2*dfacej(3, i, k, 2, 2)
        delikd = wtk1*dfacekd(3, i, j, 1, 2) + wtk2*dfacekd(3, i, j, 2, &
&          2)
...
&          max7-(abs10*delij+abs22*delik)*max7d)/max7**2
        ...
        deljid = wti1*dfaceid(3, j, k, 1, 2) + wti2*dfaceid(3, j, k, 2, &
&          2)
        delji = wti1*dfacei(3, j, k, 1, 2) + wti2*dfacei(3, j, k, 2, 2)
        deljkd = wtk1*dfacekd(3, i, j, 1, 3) + wtk2*dfacekd(3, i, j, 2, &
&          3)
...
&          max8-(abs11*delji+abs23*deljk)*max8d)/max8**2
        ...
        delkid = wti1*dfaceid(3, j, k, 1, 3) + wti2*dfaceid(3, j, k, 2, &
&          3)
        delki = wti1*dfacei(3, j, k, 1, 3) + wti2*dfacei(3, j, k, 2, 3)
        delkjd = wtj1*dfacejd(3, i, k, 1, 3) + wtj2*dfacejd(3, i, k, 2, &
&          3)
...
&          max9-(abs12*delki+abs24*delkj)*max9d)/max9**2
        ...
        del2d = delid + deljd + delkd
        del2 = deli + delj + delk
        delid = wti1*dfaceid(3, j, k, 1, 4) + wti2*dfaceid(3, j, k, 2, 4&
&          )
        deli = wti1*dfacei(3, j, k, 1, 4) + wti2*dfacei(3, j, k, 2, 4)
        deljd = wtj1*dfacejd(3, i, k, 1, 4) + wtj2*dfacejd(3, i, k, 2, 4&
&          )
        delj = wtj1*dfacej(3, i, k, 1, 4) + wtj2*dfacej(3, i, k, 2, 4)
        delkd = wtk1*dfacekd(3, i, j, 1, 4) + wtk2*dfacekd(3, i, j, 2, 4&
&          )
