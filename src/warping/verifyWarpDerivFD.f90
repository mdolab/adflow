!
! ***********************************
! *  File: verifyWarpderivFD.f90
! *  Author: C.A.(Sandy) Mader
! *  Started: 13-07-2009
! *  Modified: 13-07-2009
! ***********************************

subroutine verifyWarpDerivFD!(ncoords,xyzface,indices_new)

  ! verify the deivatives of the current mesh using FD
...
101 format("FD2Warpfile",a,".out")
  ...
  if(ierr /= 0)                        &
       call terminate("verifyWarpDeriv", &
  ...
  do ii = 1,mdNSurfNodesCompact
     ...
     do ll = 1,3
        ...
        surfacePoints(ll,ii) = surfacePointsRef(ll,ii)+deltax
...
!!$        enddo
        ...
        do nnn =1,nDom
           ...
           DO I=1,il!IMAX
              DO J=1,jl!JMAX
                 DO K=1,kl!KMAX
                    Xplus(1,I,J,K) = X(I,J,K,1)
                    Xplus(2,I,J,K) = X(I,J,K,2)
                    Xplus(3,I,J,K) = X(I,J,K,3)
                 END DO
              END DO
           END DO
...
!!$        end do
        ...
        !now perturb the other direction
        surfacePoints(ll,ii) = surfacePointsRef(ll,ii)-deltax
...
!!$        !call synchronizeBlockFaces(level,1)
        ...
        do nnn =1,nDom
           ...
           DO I=1,il!IMAX
              DO J=1,jl!JMAX
                 DO K=1,kl!KMAX
                    Xminus(1,I,J,K) = X(I,J,K,1)
                    Xminus(2,I,J,K) = X(I,J,K,2)
                    Xminus(3,I,J,K) = X(I,J,K,3)
                 END DO
              END DO
           END DO
...
!!$        deallocate(xyzref)
        ...
        !reset perturbations
        surfacePoints(ll,ii) = surfacePointsRef(ll,ii)
