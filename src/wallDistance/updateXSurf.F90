subroutine updateXSurf(level)
  ...
  call VecScatterBegin(wallScatter(level), xVolumeVec(level), xSurfVec(level), INSERT_VALUES, SCATTER_FORWARD, ierr)
  call EChk(ierr,__FILE__,__LINE__)

  call VecScatterEnd(wallScatter(level), xVolumeVec(level), xSurfVec(level), INSERT_VALUES, SCATTER_FORWARD, ierr)
  call EChk(ierr,__FILE__,__LINE__)
