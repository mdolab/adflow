! This is the original viscous flux routine: It is slightly more
! memory efficient that then current routine, *but* is does not lend
...
!
       ...
       real(kind=realType), dimension(il,jl,2) :: uux, uuy, uuz
       real(kind=realType), dimension(il,jl,2) :: vvx, vvy, vvz
       real(kind=realType), dimension(il,jl,2) :: wwx, wwy, wwz
       ...
       do j=2,jl
         do i=2,il
           ...
           u_x = fourth*(uux(i-1,j-1,k1) + uux(i,j-1,k1) &
               +         uux(i-1,j,  k1) + uux(i,j,  k1))
           u_y = fourth*(uuy(i-1,j-1,k1) + uuy(i,j-1,k1) &
               +         uuy(i-1,j,  k1) + uuy(i,j,  k1))
           u_z = fourth*(uuz(i-1,j-1,k1) + uuz(i,j-1,k1) &
               +         uuz(i-1,j,  k1) + uuz(i,j,  k1))

           v_x = fourth*(vvx(i-1,j-1,k1) + vvx(i,j-1,k1) &
               +         vvx(i-1,j,  k1) + vvx(i,j,  k1))
           v_y = fourth*(vvy(i-1,j-1,k1) + vvy(i,j-1,k1) &
               +         vvy(i-1,j,  k1) + vvy(i,j,  k1))
           v_z = fourth*(vvz(i-1,j-1,k1) + vvz(i,j-1,k1) &
               +         vvz(i-1,j,  k1) + vvz(i,j,  k1))

           w_x = fourth*(wwx(i-1,j-1,k1) + wwx(i,j-1,k1) &
               +         wwx(i-1,j,  k1) + wwx(i,j,  k1))
           w_y = fourth*(wwy(i-1,j-1,k1) + wwy(i,j-1,k1) &
               +         wwy(i-1,j,  k1) + wwy(i,j,  k1))
           w_z = fourth*(wwz(i-1,j-1,k1) + wwz(i,j-1,k1) &
               +         wwz(i-1,j,  k1) + wwz(i,j,  k1))

           q_x = fourth*(qqx(i-1,j-1,k1) + qqx(i,j-1,k1) &
               +         qqx(i-1,j,  k1) + qqx(i,j,  k1))
           q_y = fourth*(qqy(i-1,j-1,k1) + qqy(i,j-1,k1) &
               +         qqy(i-1,j,  k1) + qqy(i,j,  k1))
           q_z = fourth*(qqz(i-1,j-1,k1) + qqz(i,j-1,k1) &
               +         qqz(i-1,j,  k1) + qqz(i,j,  k1))
       ...
       kLoop: do k=2,kl
         ...
         ! The results are stored in uux(:,:,k1), etc.
...
!
         do j=1,jl
           do i=2,il
             ...
             u_x = fourth*(uux(i-1,j,k1) + uux(i,j,k1) &
                 +         uux(i-1,j,k2) + uux(i,j,k2))
             u_y = fourth*(uuy(i-1,j,k1) + uuy(i,j,k1) &
                 +         uuy(i-1,j,k2) + uuy(i,j,k2))
             u_z = fourth*(uuz(i-1,j,k1) + uuz(i,j,k1) &
                 +         uuz(i-1,j,k2) + uuz(i,j,k2))

             v_x = fourth*(vvx(i-1,j,k1) + vvx(i,j,k1) &
                 +         vvx(i-1,j,k2) + vvx(i,j,k2))
             v_y = fourth*(vvy(i-1,j,k1) + vvy(i,j,k1) &
                 +         vvy(i-1,j,k2) + vvy(i,j,k2))
             v_z = fourth*(vvz(i-1,j,k1) + vvz(i,j,k1) &
                 +         vvz(i-1,j,k2) + vvz(i,j,k2))

             w_x = fourth*(wwx(i-1,j,k1) + wwx(i,j,k1) &
                 +         wwx(i-1,j,k2) + wwx(i,j,k2))
             w_y = fourth*(wwy(i-1,j,k1) + wwy(i,j,k1) &
                 +         wwy(i-1,j,k2) + wwy(i,j,k2))
             w_z = fourth*(wwz(i-1,j,k1) + wwz(i,j,k1) &
                 +         wwz(i-1,j,k2) + wwz(i,j,k2))

             q_x = fourth*(qqx(i-1,j,k1) + qqx(i,j,k1) &
                 +         qqx(i-1,j,k2) + qqx(i,j,k2))
             q_y = fourth*(qqy(i-1,j,k1) + qqy(i,j,k1) &
                 +         qqy(i-1,j,k2) + qqy(i,j,k2))
             q_z = fourth*(qqz(i-1,j,k1) + qqz(i,j,k1) &
                 +         qqz(i-1,j,k2) + qqz(i,j,k2))
...
!
         do j=2,jl
           do i=1,il
             ...
             u_x = fourth*(uux(i,j-1,k1) + uux(i,j,k1) &
                 +         uux(i,j-1,k2) + uux(i,j,k2))
             u_y = fourth*(uuy(i,j-1,k1) + uuy(i,j,k1) &
                 +         uuy(i,j-1,k2) + uuy(i,j,k2))
             u_z = fourth*(uuz(i,j-1,k1) + uuz(i,j,k1) &
                 +         uuz(i,j-1,k2) + uuz(i,j,k2))

             v_x = fourth*(vvx(i,j-1,k1) + vvx(i,j,k1) &
                 +         vvx(i,j-1,k2) + vvx(i,j,k2))
             v_y = fourth*(vvy(i,j-1,k1) + vvy(i,j,k1) &
                 +         vvy(i,j-1,k2) + vvy(i,j,k2))
             v_z = fourth*(vvz(i,j-1,k1) + vvz(i,j,k1) &
                 +         vvz(i,j-1,k2) + vvz(i,j,k2))

             w_x = fourth*(wwx(i,j-1,k1) + wwx(i,j,k1) &
                 +         wwx(i,j-1,k2) + wwx(i,j,k2))
             w_y = fourth*(wwy(i,j-1,k1) + wwy(i,j,k1) &
                 +         wwy(i,j-1,k2) + wwy(i,j,k2))
             w_z = fourth*(wwz(i,j-1,k1) + wwz(i,j,k1) &
                 +         wwz(i,j-1,k2) + wwz(i,j,k2))

             q_x = fourth*(qqx(i,j-1,k1) + qqx(i,j,k1) &
                 +         qqx(i,j-1,k2) + qqx(i,j,k2))
             q_y = fourth*(qqy(i,j-1,k1) + qqy(i,j,k1) &
                 +         qqy(i,j-1,k2) + qqy(i,j,k2))
             q_z = fourth*(qqz(i,j-1,k1) + qqz(i,j,k1) &
                 +         qqz(i,j-1,k2) + qqz(i,j,k2))
...
!
         real(kind=realType), dimension(il,jl,2), intent(inout) :: &
                        uux, uuy, uuz, vvx, vvy, vvz, wwx, wwy, wwz, qqx, qqy, qqz
!
!        Local variables.
!
         ...
         do j=1,jl
           do i=1,il
             ...
             ! Compute the average velocities and speed of sound squared
             ! for this integration point. Node that these variables are
             ! stored in w(ivx), w(ivy), w(ivvz) and p.
             ...
             uux(i,j,k1) = uux(i,j,k1) + ubar*sx
             uuy(i,j,k1) = uuy(i,j,k1) + ubar*sy
             uuz(i,j,k1) = uuz(i,j,k1) + ubar*sz

             vvx(i,j,k1) = vvx(i,j,k1) + vbar*sx
             vvy(i,j,k1) = vvy(i,j,k1) + vbar*sy
             vvz(i,j,k1) = vvz(i,j,k1) + vbar*sz

             wwx(i,j,k1) = wwx(i,j,k1) + wbar*sx
             wwy(i,j,k1) = wwy(i,j,k1) + wbar*sy
             wwz(i,j,k1) = wwz(i,j,k1) + wbar*sz

             qqx(i,j,k1) = qqx(i,j,k1) - a2*sx
             qqy(i,j,k1) = qqy(i,j,k1) - a2*sy
             qqz(i,j,k1) = qqz(i,j,k1) - a2*sz
         ...
         do j=1,je
           do i=1,il
             ...
             if(j > 1) then
               uux(i,j-1,k1) = uux(i,j-1,k1) + ubar*sx
               uuy(i,j-1,k1) = uuy(i,j-1,k1) + ubar*sy
               uuz(i,j-1,k1) = uuz(i,j-1,k1) + ubar*sz

               vvx(i,j-1,k1) = vvx(i,j-1,k1) + vbar*sx
               vvy(i,j-1,k1) = vvy(i,j-1,k1) + vbar*sy
               vvz(i,j-1,k1) = vvz(i,j-1,k1) + vbar*sz

               wwx(i,j-1,k1) = wwx(i,j-1,k1) + wbar*sx
               wwy(i,j-1,k1) = wwy(i,j-1,k1) + wbar*sy
               wwz(i,j-1,k1) = wwz(i,j-1,k1) + wbar*sz

               qqx(i,j-1,k1) = qqx(i,j-1,k1) - a2*sx
               qqy(i,j-1,k1) = qqy(i,j-1,k1) - a2*sy
               qqz(i,j-1,k1) = qqz(i,j-1,k1) - a2*sz
             endif
         ...
         do j=1,jl
           do i=1,ie
             ...
             if(i > 1) then
               uux(i-1,j,k1) = uux(i-1,j,k1) + ubar*sx
               uuy(i-1,j,k1) = uuy(i-1,j,k1) + ubar*sy
               uuz(i-1,j,k1) = uuz(i-1,j,k1) + ubar*sz

               vvx(i-1,j,k1) = vvx(i-1,j,k1) + vbar*sx
               vvy(i-1,j,k1) = vvy(i-1,j,k1) + vbar*sy
               vvz(i-1,j,k1) = vvz(i-1,j,k1) + vbar*sz

               wwx(i-1,j,k1) = wwx(i-1,j,k1) + wbar*sx
               wwy(i-1,j,k1) = wwy(i-1,j,k1) + wbar*sy
               wwz(i-1,j,k1) = wwz(i-1,j,k1) + wbar*sz

               qqx(i-1,j,k1) = qqx(i-1,j,k1) - a2*sx
               qqy(i-1,j,k1) = qqy(i-1,j,k1) - a2*sy
               qqz(i-1,j,k1) = qqz(i-1,j,k1) - a2*sz
             endif
         ...
         do j=1,jl
           do i=1,il
             ...
             ! fluxes. The velocity gradients are stored in uux, etc.

             uux(i,j,k1) = uux(i,j,k1)*oneOverV
             uuy(i,j,k1) = uuy(i,j,k1)*oneOverV
             uuz(i,j,k1) = uuz(i,j,k1)*oneOverV

             vvx(i,j,k1) = vvx(i,j,k1)*oneOverV
             vvy(i,j,k1) = vvy(i,j,k1)*oneOverV
             vvz(i,j,k1) = vvz(i,j,k1)*oneOverV

             wwx(i,j,k1) = wwx(i,j,k1)*oneOverV
             wwy(i,j,k1) = wwy(i,j,k1)*oneOverV
             wwz(i,j,k1) = wwz(i,j,k1)*oneOverV

             qqx(i,j,k1) = qqx(i,j,k1)*oneOverV
             qqy(i,j,k1) = qqy(i,j,k1)*oneOverV
             qqz(i,j,k1) = qqz(i,j,k1)*oneOverV
