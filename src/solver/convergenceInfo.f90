...
subroutine convergenceInfo
  ...
  ! make sure iterTot is NEVER larger than ncycles.
  ...
  spectralLoop: do sps=1,nTimeIntervalsSpectral
     ...
     testRootProc: if(myID == 0) then
        ...
        if(.not.standAloneMode .and. printIterations)then
           !
           ! ---------- eran_idendifyname ------ 
           ! For CHIMPS run print identifier infront each line (code-name)
           !
        ...
        case (steady, timeSpectral)
           ...
           ! Not converged if the absCheck is True and the rel Check is true.
           if (absNotConv .and. relNotConv) then 
              converged = .False.
           end if
