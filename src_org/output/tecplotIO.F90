...
contains
  subroutine addParaSlice(sliceName, pt, direction, famList, n)
    ...
    integer(kind=intType), dimension(:), allocatable :: elemFam, cgnsBlockID, stateCellID
    ...
    do sps=1, nTimeIntervalsSpectral
       ...
       allocate(pts(3, sizeNode), conn(4, sizeCell), elemFam(sizeCell), cgnsBlockID(sizeCell), stateCellID(sizeCell))
       call getSurfaceConnectivity(conn, cgnsBlockID, stateCellID, sizeCell, wallList, size(wallList), .True.)
...
#include <petsc/finclude/petsc.h>
    ...
    integer(kind=intType), dimension(:), allocatable :: mask, elemFam, localElemFam, cgnsBlockID, stateCellID
    ...
    spectralLoop: do sps=1,nTimeIntervalsSpectral
       ...
       masterBCLoop: do iBCGroup=1,nFamExchange
          ...
          allocate(localConn(4, sizeCell), localElemFam(sizeCell), cgnsBlockID(sizeCell), stateCellID(sizeCell))
          call getSurfaceConnectivity(conn, cgnsBlockID, stateCellID, sizeCell, exch%famList, size(exch%famList), .True.)
          call getSurfaceFamily(localElemFam, sizeCell, exch%famList, size(exch%famList), .True.)
