...
contains
  ...
  subroutine getSurfaceConnectivity(conn, cgnsBlockID, stateCellID, ncell, famList, nFamList, includeZipper)
    ...
    use constants
    use blockPointers, only : nDom, nBocos, BCData, BCFaceID, rightHanded, nbkGlobal, il, jl, kl
    use utils, only : setPointers
    use sorting, only : famInList
    use surfaceFamilies, only : BCFamGroups
    use oversetData, only : zipperMeshes, zipperMesh, oversetPresent
    ...
    integer(kind=intType), intent(inout) :: conn(4*ncell), cgnsBlockID(ncell), stateCellID(ncell)
    integer(kind=intType), intent(in) :: nFamList, famList(nFamList)
    logical, intent(in) :: includeZipper

    ! Working
    integer(kind=intType) :: nn, mm, cellCount, nodeCount, domCount, ni, nj, i, j
    integer(kind=intType) :: inow, jnow, know
    integer(kind=intType) :: iBeg, iEnd, jBeg, jEnd, iBCGroup
    logical regularOrdering, BCGroupNeeded
    type(zipperMesh), pointer :: zipper
    cellCount = 0
    nodeCount = 0
    domCount = 0 ! This will store the offset in the state vector for each flowdom
    ...
    domains: do nn=1,nDom
                ...
                do j=0,nj-2
                   do i=0,ni-2
                      if (.not. includeZipper .or. BCData(mm)%iBlank(i+iBeg+1, j+jBeg+1) ==1 ) then
                         ...
                         ! Generate the state vector ID
                         ! This will not take the quantity of state variables into
                         ! account. We only use the i,j,k indices
                         !
                         ! First we need to find the actual i,j,k indices
                         ! of the cell, since BCData considers i and j only
                         ! for all faces
                         select case(BCFaceID(mm))
                         ...
                         ! il,jl,kl are the last cell IDs in the block
                         stateCellID(cellCount+1) = domCount + inow + (il - 1)*(jnow-1) &
                                                    + (il-1)*(jl-1)*(know-1)
       ...
       ! Update the block cell offset
       domCount = (il-1)*(jl-1)*(kl-1)
