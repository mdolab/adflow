...
parser.add_argument(
    ...
    type=int,
    nargs="+",
    help="Specify which procs you want to check the loadbalacning for. This can be a single number or list of numbers",
)
parser.add_argument(
    "--sequence",
    action="store_true",
    help="If specified, the first two arguments from procs argument are used to create a range where all proc numbers in btween are tested.",
)
parser.add_argument(
    "--mgCycle",
    type=str,
    default="2w",
    help="Set the level of multigrid intended to run on mesh [default: %(default)s]",
)
parser.add_argument(
    "--outfile",
    type=str,
    help="Set the output filename for the analysis. Default: filename is generated from mesh name and other input.",
)
...
# Define problem
aeroOptions = {"gridFile": args.grid, "mgcycle": args.mgCycle, "partitionOnly": True}
